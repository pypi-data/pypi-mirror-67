...
ARGUMENT_TYPE_TO_PY_TYPE = {
    'text': (
        str,
        lambda s: True,
        'Free text. Can contain letters, numbers, symbols, etc..'
    ),
    'alphabetic': (
        str,
        lambda s: re.match('[a-zA-Z]+', s) is not None,
        'Alphabetic string. Contains letters only.'
    ),
    'alphanumeric': (
        str,
        lambda s: re.match('[a-zA-Z0-9]+', s) is not None,
...
def check_valid_argument_value(param_name, param_info, value):
    ...
    if param_type not in ARGUMENT_TYPE_TO_PY_TYPE:
        raise ValueError(
            "Value for API argument '{}' of type '{}' is not valid".format(param_name, param_type)
        )
    # ---
