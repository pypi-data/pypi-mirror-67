...
class PYConnection: 
    ...
    def __init__(self, ConString): 
        ...
        from quarchpy.device.scanDevices import get_connection_target
        ...
        if self.ConnTypeStr.lower() == 'rest': 
            ...
            if "qtl" in self.ConnTarget.lower():
                self.ConnTarget = get_connection_target(self.ConnTarget, connection_preference="REST", include_conn_type = False )
            ...
            if "qtl" in self.ConnTarget.lower():
                self.ConnTarget = get_connection_target(self.ConnTarget, connection_preference="TELNET", include_conn_type = False)
            ...
            if "qtl" in self.ConnTarget.lower():
                self.ConnTarget = get_connection_target(self.ConnTarget, connection_preference="TCP", include_conn_type = False)
