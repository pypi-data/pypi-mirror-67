...
class Analyzer(ABC):
    ...
    def get_topology(acq_config_xml):
        ...
        return json.loads(chain.get_dif_topology(acq_config_xml.encode('utf-8')))

    def multiple_file_loop(self, input_files, chains=None):
    ...
    def single_file_loop(self, chains=None):
        ...
        if dummy_id not in chains:
            ...
            previous_id = id(chains[dummy_id].link)
            chains[dummy_id].thread = chains[dummy_id].link.spawn(self.name, **self.args)
        if previous_id is not None and id(chains[dummy_id].link) != previous_id:
            raise ReferenceError("Duh")
...
class Decoder(Analyzer, ABC):
    ...
    def _set_runtime_arguments(self, input_file):
        ...
        self.args["n_chips"] = len(self.get_topology(self.acq_config_xml)[str(dif_id)])
...
class SpillNumberFixer(Analyzer, ABC):
    ...
    _default_args.update({'flipped_bits': "", 'offset': 0})
