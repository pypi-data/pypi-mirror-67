...
LiszItem = Dict[str, Any]               #: item data (lid) type
...
class LiszDataMixin(ABC):
    ...
    def edit_validate(self, old_idx: int, new_id: str, want_node: bool) -> str:
        ...
        :param want_node:       True if the new/edited item will have a sub-node.
        ...
        if add_item:                        # add new item
            ...
            lid['id'] = new_id
            if want_node != ('node' in lid):    # NOTE: != operator has lower priority than in operator
                if want_node:
                    lid['node'] = list()        # type: ignore   # mypy not supports recursive types see issue #731
                elif lid['node']:       # let user confirm node deletion of non-empty lid['node']
    ...
    def find_item_index(self, item_id: str, searched_node: Optional[LiszNode] = None) -> int:
        ...
        if searched_node is None:
            searched_node = self.current_node
        for list_idx, lid in enumerate(searched_node):
            if lid['id'] == item_id:
                return list_idx
        return -1
    ...
    def flow_path_node(self, flow_path: List[str] = None) -> LiszNode:
        ...
        for flow_id in flow_path:
            if flow_action(flow_id) != 'enter':
                break
            node_id = flow_key(flow_id)
            current_node = self.item_by_id(node_id, searched_node=current_node)['node']
        return current_node
    ...
    def on_app_init(self):
        ...
        self.refresh_current_node_from_flow_path()
    ...
    def on_key_press(self, modifiers: str, key_code: str) -> bool:
        ...
        if key_code == 'up':
            self.set_neighbour_flow(-1)
        elif key_code == 'down':
            self.set_neighbour_flow(1)
        elif key_code == 'pgup':
            self.set_neighbour_flow(-15)
        elif key_code == 'pgdown':
            self.set_neighbour_flow(15)
        elif key_code == 'home':
            self.set_neighbour_flow(-999999)
        elif key_code == 'end':
            self.set_neighbour_flow(999999)

        # toggle selection of current item
        elif key_code == ' ' and self.flow_id:    # key string 'space' is not in Window.command_keys
            self.toggle_item_sel(self.find_item_index(flo_key))
            self.refresh_node_widgets()

        # enter/leave flow in current list
        elif key_code in ('enter', 'right') and self.flow_id and 'node' in self.item_by_id(flo_key):
            self.change_flow(id_of_flow('enter', 'node', flo_key))
        elif key_code in ('escape', 'left') and self.flow_path:
            self.change_flow(id_of_flow('leave', 'node'))

        # item processing: add, edit or request confirmation of deletion of current item
        elif key_code in ('a', '+'):
            self.change_flow(id_of_flow('add', 'item'))

        elif key_code == 'e' and self.flow_id:
            self.change_flow(replace_flow_action(self.flow_id, 'edit'))  # popup_kwargs=dict(parent=self.framework_win))

        elif key_code in ('-', 'del') and self.flow_id:
            self.change_flow(id_of_flow('confirm', 'item_deletion'), item_id=flo_key)
    ...
    def on_node_enter(self, _flow_id: str, _event_kwargs: dict):
        ...
        """
        self.play_sound(id_of_flow('enter', 'node'))

    def on_node_leave(self, _flow_id: str, _event_kwargs: dict):
        ...
        :return:                True for to process/change flow id.
        """
        self.play_sound(id_of_flow('leave', 'node'))
    ...
    def refresh_all(self):
        ...
        redraw = False
        if flow_action(flow_id) == 'focus' and self.find_item_index(flow_key(flow_id)) == -1:
            flow_id = id_of_flow('', '')    # reset flow id because last focused item got filtered/deleted by user
            redraw = True

        self.change_app_state('flow_id', flow_id, send_event=redraw)    # correct flow or restore silently
    ...
    def refresh_current_node_from_flow_path(self):
        """ refresh current node including the depending display node. """
        self.current_node = self.flow_path_node()

    def set_neighbour_flow(self, delta):
        """ move flow id to previous/next item. """
        current_node = self.current_node
        if current_node:
            flow_id = self.flow_id
            if flow_id:
                idx = min(max(0, self.find_item_index(flow_key(flow_id)) + delta), len(current_node) - 1)
            else:
                idx = min(max(-1, delta), 0)
            self.change_app_state('flow_id', id_of_flow('focus', 'item', current_node[idx]['id']))
    ...
    def toggle_item_sel(self, node_idx: int):
        ...
        """
        new_sel = not self.current_node[node_idx].get('sel', False)
        if new_sel:
