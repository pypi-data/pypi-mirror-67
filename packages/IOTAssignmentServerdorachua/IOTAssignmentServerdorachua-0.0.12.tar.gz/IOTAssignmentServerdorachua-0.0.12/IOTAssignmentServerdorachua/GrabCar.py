...
class GrabCar:
  ...
  def read(self, readtime):
    try:
      ...
      mysqlm = MySQLManager.MySQLManager(self.user,self.password,self.host,self.database)
      ...
      if result == True:
        ...
        result = mysqlm.retrieve(MySQLManager.QUERYTYPE_RETRIEVE, sql,data)
        if result == True:
          for (bookingid,accuracy,bearing,acceleration_x,acceleration_y,acceleration_z,gyro_x,gyro_y,gyro_z,seconds,speed,speedkmhour) in mysqlm.cursor:
            ...
            return json.dumps(output,cls=MyJSONHelper.OurCustomEncoder)
        else:
