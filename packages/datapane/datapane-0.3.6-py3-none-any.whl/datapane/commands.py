...
def failure_msg(msg: str, exit: bool = False):
...
def deploy(title: Optional[str], script: Optional[str], config: Optional[str], visibility: str):
    ...
    with click_spinner.spinner():
        ...
        # build in the cur dir (for now)
        proj_dir = config and config.parent
        sdist = scripts.build_local_sdist(dp_cfg, proj_dir=proj_dir)
    ...
    if wait:
        with click_spinner.spinner():
            ...
            if r.status == "SUCCESS":
                ...
                print(r.error_msg)
                failure_msg(f"Script run failed/cancelled", exit=True)
