...
class DatapaneCfg:
    ...
    @property
    def entrypoint(self):
        # NOTE  - relies on ".py" extension
        return ".".join(self.script.parts)[:-3]
    ...
    def to_dict(self, filter_build=False) -> SDict:
        d = dc.asdict(self)
        # TODO - make script a getter/setter
        d["script"] = str(d["script"])  # this is hacky - need a better DTO-conversion method
        build_fields = {"include", "exclude", "requirements"}
        return {k: v for k, v in d.items() if k not in build_fields} if filter_build else d
