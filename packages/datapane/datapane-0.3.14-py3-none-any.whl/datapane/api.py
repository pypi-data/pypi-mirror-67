...
class Script(BEObjectRef):
    ...
    def upload_pkg(cls, sdist: Path, dp_cfg: DatapaneCfg, **kwargs) -> "Script":
        ...
        new_kwargs = {
            **dp_cfg.to_dict(filter_build=True),
            **dict(build_cfg=dp_cfg.to_dict()),
            **kwargs,
        }
...
def cleanup_tmp():
    dp_tmp = Path("dp-tmp")
    for child in dp_tmp.glob("*"):
        # We expect child to always be a file, not a dir. In the case of dirs we'd need to recursively remove.
        child.unlink()
