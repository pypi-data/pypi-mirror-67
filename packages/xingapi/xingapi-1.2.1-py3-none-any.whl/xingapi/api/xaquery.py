    ...
    def wait(self):
        ...
        if count==limit:
            ...
            while True:
                ...
                time.sleep(0.1)
                print(f'조회제한 해제까지 남은시간 {timedelta(seconds=remaining)}', end='\r')
        ...
        while self.isnext:
            ...
            for block_name, block_data in self.data.items():
                for input_key, next_key in keypairs.items():
                    ...
                    #     next_input_kwargs[input_key] = value[0]

            # for input_key, (next_block, next_key) in keypairs.items():
            #     next_label = self.res.blocks[next_block].get(next_key)
            #     next_input_kwargs[input_key] = self.data[next_block][next_label][0]
