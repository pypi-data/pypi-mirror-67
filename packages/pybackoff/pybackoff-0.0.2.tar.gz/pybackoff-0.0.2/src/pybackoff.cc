...
public:
    ...
    size_t getSize();
    std::map<uint64_t, int> getCountsMap();
    Ngrams();
...
public:
    ...
    SBModel(const std::map<uint64_t, int>& counts, size_t N);
...
std::map<uint64_t, int> Ngrams::getCountsMap() {
    return counts;
}
...
SBModel::SBModel(const std::map<uint64_t, int>& counts, size_t N) {
    this->counts = counts;
    this->N = N;
}
...
PYBIND11_MODULE(pybackoff, m) {
    ...
    py::class_<SBModel>(m, "SBModel")
        .def(py::init<const std::map<uint64_t, int>&, size_t>())
