...
class CompoundParser:
    ...
    def parse(self):
        ...
        for line in self.file:
            # Comments are sometimes malformed
...
class EnzymeParser:
    ...
    def __init__(self, path, parser_options=None):
        self.options = {}
        if parser_options is not None:
            self.options.update(parser_options)
        self.file = _open_file_and_remove_comments(path)
        self.actions = {
...
class Repairer:
    ...
    def unify_reaction_direction(self, reaction):
        ...
        if reaction["reversible"]:
            ...
            if reaction["direction"] in (
                ...
                raise ValueError(f"Weird reaction direction for reaction {reaction}")
    ...
    def split_location_string(self, location_string):
        ...
        """
        split = re.split("(\-?CCO\-)", location_string)
        try:
            out_, in_ = split[2::2]
        except ValueError:
            out_ = split[2]
