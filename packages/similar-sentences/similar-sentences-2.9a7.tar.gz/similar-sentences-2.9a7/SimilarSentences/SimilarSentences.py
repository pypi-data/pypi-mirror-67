...
class SimilarSentences:
        ...
        if(os.path.isfile(file_path) and self.get_file_extension(file_path) == ".txt"):
            ...
            with open(file_path) as bs: 
                ...
                for sentence in sentences: 
                    ...
                    #self.create_excel(output, len(self.get_sentences(sentence)))                
        ...
        while i < closest_n:
            ...
            for query, sentence in zip(text, embedding):
                ...
                for idx, distance in results[0:closest_n]:
                    if(i == index):
                        out.append(
                            {"sentence": sentences[idx].strip(), "score": (1-distance), "given":sentence})
                    index += 1
                output[i] = out
            i += 1
            if(response == "batch"):
    ...
    def create_excel(self, data, sentence_length):
        row, col, i = 0
        print("Data {} and length of sentence {}".format(data,sentence_length))
        workbook = xlsxwriter.Workbook('Results.xlsx')
        worksheet = workbook.add_worksheet()
        while i < sentence_length:
            for d in data:
                for j in d:
                    worksheet.write(row, col, data[j][i])
                    worksheet.write(row, col + 1, data[j][i])
                    worksheet.write(row, col + 2, data[j][i])
                    row += 1
            i += 1
        workbook.close()
