...
class Auto_indb():
    def __init__(self, host='rm-2zeo0ew8s18q793q32o.mysql.rds.aliyuncs.com', username='hd_woca', password='',
                 db='hd_one', table_name='', comment='', create_tables=True):
        red_print("--------------------------------------------------------------------------------------------")
        print(
...
class Auto_sinsert():
    def __init__(self, host='127.0.0.1', username='root', password='mysql', db='test',
        ...
        print("--------------------------------------------------------------------------------------------")
        print(
            '''auto_sinsert = Auto_sinsert(host='192.168.4.201',username='root',password='mysql',db='zhijianju',drop_column=["id","jid","updated","entid"])''')
        print("--------------------------------------------------------------------------------------------")
        self.host = host
        self.username = username
        self.password = password
...
def my_request(url='', conn=requests, proxy=None, headers={
    ...
    while True:
        ...
        try:
            ...
            if log_record:
                red_print("本次请求失败,重试次数剩余：{} ,错误日志记录位置：{}".format(retry - request_count,error_path))
                ...
                red_print("本次请求失败,重试次数剩余：{} ".format(retry - request_count))
            proxy = get_ip()
        request_count += 1
