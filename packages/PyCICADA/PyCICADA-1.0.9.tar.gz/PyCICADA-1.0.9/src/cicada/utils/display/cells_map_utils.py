...
from cicada.utils.graphs.utils_graphs import welsh_powell
...
class CellsCoord:
    ...
    def match_cells_indices(self, coord_obj, path_results, max_dist_bw_centroids=3, plot_result=True, plot_title_opt="",
        ...
        for cell in np.arange(len(coord_obj.coord)):
            ...
            if np.min(distances) <= max_dist_bw_centroids:
                if take_the_biggest:
                    # look for 3 nearest
                    args_sort = np.argsort(distances)[:3]
                    biggest_area = -1
                    biggest_one_index = 0
                    for cell_index in args_sort:
                        area = self.cells_polygon[cell_index].area
                        if area > biggest_area:
                            biggest_area = area
                            biggest_one_index = cell_index
                    mapping_array[cell] = biggest_one_index
                else:
...
def get_coords_from_caiman_format_file(file_name, key_to_data=None):
    ...
    the cell. If type_of_coords is in suite2p mode, then a  pixel_masks format is returned
