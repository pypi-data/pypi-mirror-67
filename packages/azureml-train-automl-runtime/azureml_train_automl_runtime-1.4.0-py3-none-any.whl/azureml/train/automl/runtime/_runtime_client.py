...
from azureml.automl.runtime._offline_automl_run import (
    _OfflineAutoMLRun, _OfflineAutoMLRunContext, _OfflineAutoMLRunUtil)
...
from .utilities import _calculate_start_end_times, _create_best_run, \
    _upload_summary_of_iterations
...
class RuntimeClient:
    ...
    def _fit_local(self, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None, sample_weight_valid=None,
        ...
        try:
            ...
            # If not tracking child runs individually in RH
            if self.azureautomlclient.automl_settings.track_child_runs is False and self._run_best:
                ...
                _create_best_run(
                    ...
                    best_offline_run=cast(_OfflineAutoMLRun, self._run_best),
                    logger=self.azureautomlclient.logger)
    ...
    def _fit_iteration(self,
        ...
        try:
            ...
            if isinstance(child_run, Run):
                ...
                # If not tracking child runs, the offline run must instead be passed from parent to any child process
                # that requires it directly, instead of recreated using these environment variables. (For that reason,
                # an offline run must be pickleable, but regular runs may not be.)
    ...
    def _get_child_run_and_context(
    ...
    ) -> Tuple[Union[Run, _OfflineAutoMLRun], Union[AzureAutoMLRunContext, _OfflineAutoMLRunContext]]:
        ...
        if self.azureautomlclient.automl_settings.track_child_runs:
            ...
            offline_child_run = _OfflineAutoMLRunUtil.create_child_run(
            ...
            offline_run_context = _OfflineAutoMLRunContext(offline_child_run)
