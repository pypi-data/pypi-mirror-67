...
def automl_setup_model_explanations(fitted_model: Union[Pipeline, StreamingPipelineWrapper], task: str,
    ...
    if task == constants.Tasks.CLASSIFICATION and y is not None:
        ...
        classes = _get_unique_classes(y).tolist()  # type: Optional[List[Any]]
    else:
        classes = None

    surrogate_model, surrogate_model_params = _automl_pick_surrogate_model_and_set_params(
        X_test_transform,
        _get_num_physical_cpu_cores_model_explanations())
...
def _automl_pick_surrogate_model_and_set_params(explainer_test_data: DataInputType,
                                                num_cpu_cores: int, run_algorithm_name: Optional[str] = None,
    ...
    :param run_algorithm_name: Name of the model being explained.
    :type run_algorithm_name: str
    :param logger: Logger for info/error messages.
    :type logger: logger
    ...
    if run_algorithm_name is not None and run_algorithm_name == constants.SupportedModels.Forecasting.AutoArima:
...
def _automl_auto_mode_explain_model(child_run: Any,
    ...
    surrogate_model, surrogate_model_params = \
        _automl_pick_surrogate_model_and_set_params(explainer_test_data,
                                                    _get_num_physical_cpu_cores_model_explanations(
