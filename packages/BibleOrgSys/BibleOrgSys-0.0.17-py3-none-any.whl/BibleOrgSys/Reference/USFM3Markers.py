    ...
    def getMarkerListFromText( self, text, includeInitialText=False, verifyMarkers=False ):
        ...
        for j, (m, ix, x, mx) in enumerate(firstResult):
            if self.isNewlineMarker( m ):
                if cx:
                    logging.critical( f"USFM3Markers.getMarkerListFromText: ABOUT1 TO CLEAR {cx} after {j} {m} {ix} {x} {mx}" )
                ...
                and cx[0] not in ('f','fr','fq') and m not in ('fr','fq','ft'):
                    logging.critical( f"USFM3Markers.getMarkerListFromText: ABOUT2 TO CLEAR {cx} after {j} {m} {ix} {x} {mx}" )
                    if debuggingThisModule or BibleOrgSysGlobals.strictCheckingFlag and BibleOrgSysGlobals.debugFlag:
                ...
                if cx: cx.pop() #; vPrint( 'Quiet', debuggingThisModule, "del", m, cx )
                    ...
                    logging.critical( f"USFM3Markers.getMarkerListFromText: WHY IS CX EMPTY WHEN ABOUT TO POP {cx} after {j} {m} {ix} {x} {mx}???" )
                    if debuggingThisModule or BibleOrgSysGlobals.strictCheckingFlag and BibleOrgSysGlobals.debugFlag:
                        assert cx
            elif x=='*':
                if len(cx) != 1 \
                or (cx[0]!=m and m!='ff' and cx[0] not in ('ft',) ):
                    logging.critical( f"USFM3Markers.getMarkerListFromText: ABOUT3 TO CLEAR {cx} after {j} {m} {ix} {x} {mx}" )
                    if debuggingThisModule or BibleOrgSysGlobals.strictCheckingFlag and BibleOrgSysGlobals.debugFlag:
