"""Script that build and save a database postal code to circonscription (french word for district).

Needed data:
    - districts (french: circonscription), downloaded from
    https://www.data.gouv.fr/fr/datasets/circonscriptions-legislatives-table-de-correspondance-des-communes-et-des-cantons-pour-les-elections-legislatives-de-2012-et-sa-mise-a-jour-pour-les-elections-legislatives-2017/
...
External sources:
    - http://www.elections-legislatives.fr/circonscriptions/075.asp?#lis which provided us the mean
    to know precisely wich arrondissement matched which circonscription for Paris and Marseilles,
    and https://medium.com/@filgb/la-carte-des-circonscriptions-de-lyon-et-du-rh%C3%B4ne-pour-les-l%C3%A9gislatives-9a29d969c8e7
    for Lyon

Output:
    - postal code to district number dict, saved as json object into postal_code_to_district.json
...
# Raw data : we store here data from cities where the districts better correspond to postal codes
# than to insee codes. These dict map arrondissements to their district numbers. The function
# add_static_part build the corresponding postal code -> district number
...
class District:
    ...
    def __init__(self, code_dpt, nom_dpt, code_commune, nom_commune, code_circ_legislative,
        ...
        if type(code_dpt) == str and code_dpt[0] == 'Z':
            self.code_dpt, self.code_commune = dpt_commune_to_int(code_dpt, code_commune)
            self.key = "{}.{}".format(self.code_dpt * 10 + (self.code_commune // 100),
...
def import_insee_postal() -> Dict[str, str]:
    """Build a correspondance between insee code and postal codes from insee_postal.csv"""
    result = dict()
...
def bind(districts: Dict[str, List[District]], insee: Dict[str, str]) \
    ...
    result = dict((key, tuple(value)) for key, value in found.items())
