...
class WriteOnlyDataset:
    ...
    def __init__(self, data_dir, data_name, record_type):
        ...
        self._meta_db = _MetaDB(os.path.join(self._data_dir, "{}.prop".format(self._data_name)))
...
class _MetaDB:
    ...
    def merge_from(self, other):
        assert isinstance(other, _MetaDB), "should be MetaDB object. but get `{}`".format(type(other))
        for k in other.keys():
            if self.has(k):
