...
from mingdongnlp import pretrained
from mingdongnlp.common.component import Component
from mingdongnlp.utils.io_util import get_resource, load_json, eprint
from mingdongnlp.utils.reflection import object_from_class_path, str_to_type
from mingdongnlp import version
...
def load_from_meta_file(save_dir: str, meta_filename='meta.json', transform_only=False, load_kwargs=None,
    ...
    try:
        ...
        if model_version != cur_version:
            eprint(
                f'{identifier} was created with mingdongnlp-{model_version}, while you are running {cur_version}. '
                f'Try to upgrade mingdongnlp with\n'
                f'pip install --upgrade mingdongnlp\n'
                f'If the problem persists, please submit an issue to https://github.com/hankcs/mingdongnlp/issues .')
        exit(1)
