...
from mingdongnlp.utils import time_util
from mingdongnlp.utils.log_util import logger
from mingdongnlp.utils.string_util import split_long_sentence_into
from mingdongnlp.utils.time_util import now_filename
from mingdongnlp.common.constant import HANLP_URL
...
def hanlp_home_default():
    ...
    """
    if windows():
        return os.path.join(os.environ.get('APPDATA'), 'mingdongnlp')
    else:
        return os.path.join(os.path.expanduser("~"), '.mingdongnlp')
...
def download(url, save_path=None, save_dir=hanlp_home(), prefix=HANLP_URL, append_location=True):
    ...
    if os.path.isfile(save_path):
        ...
        try:
            ...
            opener.addheaders = [('User-agent', f'mingdongnlp/{version.__version__}')]
