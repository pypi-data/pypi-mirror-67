...
import mingdongnlp.callbacks
import mingdongnlp.common
import mingdongnlp.components
import mingdongnlp.datasets
import mingdongnlp.layers
import mingdongnlp.losses
import mingdongnlp.metrics
import mingdongnlp.optimizers
import mingdongnlp.pretrained
import mingdongnlp.utils

from mingdongnlp.version import __version__

if not os.environ.get('HANLP_GREEDY_GPU', None):
    exec('from mingdongnlp.utils.tf_util import nice_gpu')
    exec('nice_gpu()')

exec('''
from mingdongnlp.utils.util import ls_resource_in_module
ls_resource_in_module(mingdongnlp.pretrained)
''')


def load(save_dir: str, meta_filename='meta.json', transform_only=False, load_kwargs=None,
         **kwargs) -> mingdongnlp.common.component.Component:
    """
    ...
    """
    save_dir = mingdongnlp.pretrained.ALL.get(save_dir, save_dir)
    from mingdongnlp.utils.component_util import load_from_meta_file
    return load_from_meta_file(save_dir, meta_filename, transform_only=transform_only, load_kwargs=load_kwargs, **kwargs)


def pipeline(*pipes) -> mingdongnlp.components.pipeline.Pipeline:
    """
    Creates a pipeline of components.
    :param pipes: Components if pre-defined any.
    :return: A pipeline
    """
    return mingdongnlp.components.pipeline.Pipeline(*pipes)
