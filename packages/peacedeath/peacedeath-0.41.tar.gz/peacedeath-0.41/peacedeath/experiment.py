...
def write_parameters_drug_doses(pe, drug_doses=(0, 0, 0, 0, 0, 0, 0)):
    assert len(drug_doses) == 7
    with open(os.path.join(pe.name, "parameters_dose.csv"), "w+") as f:
        f.write(",".join([str(d) for d in drug_doses]))
...
def start_experiment(pe, total_dilution_volume=10, mixer_speed=30, active_tubes=()):
    ...
    schedule.every().minute.at(":15").do(dilute_if_necessary, pe, total_dilution_volume=total_dilution_volume)
