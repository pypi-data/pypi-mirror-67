...
from huajiweb.errors import huajiwebAPIWarning
from huajiweb.errors import huajiwebDeprecationWarning
from huajiweb.hashing import Context
from huajiweb.hashing import update_hash
from huajiweb.hashing import HashReason
from huajiweb.logger import get_logger
from huajiweb.util import functools_wraps
...
def maybe_show_cached_st_function_warning(dg, st_func_name):
    ...
    st_func_name : str
        The name of the huajiweb function that was called.
...
def _read_from_disk_cache(key):
    path = file_util.get_huajiweb_file_path("cache", "%s.pickle" % key)
    try:
        with file_util.huajiweb_read(path, binary=True) as input:
...
def _write_to_disk_cache(key, value):
    ...
    try:
        with file_util.huajiweb_write(path, binary=True) as output:
...
def cache(
    ...
    ----------
    func : callable
        The function to cache. huajiweb hashes the function and dependent code.

    persist : boolean
        Whether to persist the cache on disk.

    allow_output_mutation : boolean
        huajiweb normally shows a warning when return values are not mutated, as that
    ...
    suppress_st_warning : boolean
        Suppress warnings about calling huajiweb functions from within
        the cached function.

    hash_funcs : dict or None
        Mapping of types or fully qualified names to hash functions. This is used to override
        the behavior of the hasher inside huajiweb's caching mechanism: when the hasher
    ...
    # executed only once. But in huajiweb, we reload all modules related to a
...
def get_cache_path():
    return file_util.get_huajiweb_file_path("cache")
...
class CachedStFunctionWarning(huajiwebAPIWarning):
    ...
    def _get_message(self, st_func_name, cached_func):
        ...
        return (
            """
Your script uses %(st_func_name)s to write to your huajiweb app from within
...
class CachedObjectMutationWarning(huajiwebAPIWarning):
...
By default, huajiweb's cache should be treated as immutable, or it may behave
in unexpected ways. You received this warning because huajiweb detected
...
(https://docs.huajiweb.io/advanced_caching.html)
            """
