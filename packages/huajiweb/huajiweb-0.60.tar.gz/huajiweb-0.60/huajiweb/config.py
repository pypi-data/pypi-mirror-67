...
from huajiweb import development
from huajiweb import env_util
from huajiweb import file_util
from huajiweb import util
from huajiweb.ConfigOption import ConfigOption
...
# file be the same order as the sections appear with `huajiweb config show`)
...
def get_option(key):
    """Return the current value of a given huajiweb config option.
...
def _create_option(
    ...
    is requested. To prevent this, use the `huajiweb.util.memoize` decorator as
    follows:
...
_create_section("global", "Global options that apply across all of huajiweb.")

_create_option(
    "global.disableWatchdogWarning",
    description="""
        By default, huajiweb checks if the Python watchdog module is available
        and, if not, prints a warning asking for you to install it. The watchdog
        module is not required, but highly recommended. It improves huajiweb's
...
_create_option(
    ...
    description="""
        ...
        - "file" : share to a directory on the local machine. This is
          meaningful only for debugging huajiweb itself, and shouldn't be
          used for production.
        """,
    default_val="off",
)
...
_create_option(
    ...
    description="""
        If True, will show a warning when you run a huajiweb-enabled script
...
def _global_development_mode():
    ...
    This option defaults to True if and only if huajiweb wasn't installed
    normally.
    """
    return (
        not env_util.is_pex()
...
_create_option(
    ...
    description="Hide deprecation warnings in the huajiweb app.",
    visibility="hidden",
    default_val=False,
    type_=bool,
)
...
_create_section("client", "Settings for scripts that use huajiweb.")
...
_create_option(
    ...
    description="""If false, makes your huajiweb script not draw to a
        huajiweb app.""",
    default_val=True,
    type_=bool,
    scriptable=True,
...
_create_section("runner", "Settings for how huajiweb executes your script")
...
_create_option(
    ...
    description="""
        Sets the MPLBACKEND environment variable to Agg inside huajiweb to
...
_create_section("server", "Settings for the huajiweb server")
...
_create_option(
    ...
    description="""
        Change the type of file watcher used by huajiweb, or turn it off
        completely.

        Allowed values:
        * "auto"     : huajiweb will attempt to use the watchdog module, and
                       falls back to polling if watchdog is not available.
        * "watchdog" : Force huajiweb to use the watchdog module.
        * "poll"     : Force huajiweb to always use polling.
        * "none"     : huajiweb will not watch files.
    """,
    default_val="auto",
    type_=str,
)
...
def _server_headless():
    ...
    is_running_in_editor_plugin = (
        os.getenv("IS_RUNNING_IN_huajiweb_EDITOR_PLUGIN") is not None
    )
    return (
...
_create_option(
    "server.baseUrlPath",
    description="""
        The base path for the URL where huajiweb should be served from.
        """,
    default_val="",
    type_=str,
)
...
def _gather_usage_stats():
    """Whether to send usage statistics to huajiweb.
...
_create_option(
    "mapbox.token",
    description="""Configure huajiweb to use a custom Mapbox
                token for elements like st.deck_gl_chart and st.map. If you
                don't do this you'll be using huajiweb's own token,
...
def _s3_url():
    """URL root for external view of huajiweb apps.
...
def is_manually_set(option_name):
    ...
    return get_where_defined(option_name) not in (
        ...
        ConfigOption.huajiweb_DEFINITION,
    )
...
def parse_config_file(force=False):
    ...
    # Read ~/.huajiweb/config.toml, and then overlay
    # $CWD/.huajiweb/config.toml if it exists.
    config_filenames = [
        file_util.get_huajiweb_file_path("config.toml"),
        file_util.get_project_huajiweb_file_path("config.toml"),
    ]
...
def _check_conflicts():
    ...
    if get_option("global.sharingMode") == "s3":
        ...
        if is_manually_set("s3.url"):
            ...
            # the hostname of the huajiweb instance to the configured URL.)
        ...
        # to do.
        LOGGER.warning(
            "'sharingMode' is set to 'file', but huajiweb is not configured "
            "for development. This sharingMode is used for debugging "
            "huajiweb itself, and is not supported for other use-cases. "
            "\n\nTo remove this warning, set the 'sharingMode' option to "
            "another value, or remove it from your huajiweb config."
        )
