...
def add_magic(code, script_path):
    """Modifies the code to support magic huajiweb commands.
...
def _modify_ast_subtree(tree, body_attr="body", is_root=False):
    ...
    if is_root:
        # Import huajiweb so we can use it in the new_value above.
...
def _insert_import_statement(tree):
    """Insert huajiweb import statement at the top(ish) of the tree."""

    st_import = _build_st_import_statement()

    # If the 0th node is already an import statement, put the huajiweb
    ...
    # put the huajiweb import below those, so we don't break "from
    # __future__ import".
    elif (
        len(tree.body) > 1
...
def _build_st_import_statement():
    """Build AST node for `import huajiweb as __huajiweb__`."""
    return ast.Import(names=[ast.alias(name="huajiweb", asname="__huajiweb__")])


def _build_st_write_call(nodes):
    """Build AST node for `__huajiweb__._transparent_write(*nodes)`."""
    return ast.Call(
        func=ast.Attribute(
            attr="_transparent_write",
            value=ast.Name(id="__huajiweb__", ctx=ast.Load()),
            ctx=ast.Load(),
        ),
        args=nodes,
        keywords=[],
