...
from huajiweb.errors import huajiwebAPIException, MarkdownFormattedException
from huajiweb.folder_black_list import FolderBlackList
...
_CYCLE_PLACEHOLDER = b"huajiweb-57R34ML17-hesamagicalponyflyingthroughthesky-CYCLE"
...
class _CodeHasher:
    ...
    def _to_bytes(self, obj, context):
        ...
        if _is_magicmock(obj):
            ...
            if obj.__module__.startswith("huajiweb"):
                # Ignore huajiweb modules even if they are in the CWD
            ...
            # st.warning(('huajiweb does not support hashing modules. '
            ...
            # quite annoying...
            # st.warning(('huajiweb does not support hashing classes. '
            #             'We did not hash `%s`.') % obj.__name__)
...
class UnhashableTypeError(huajiwebAPIException):
...
While caching %(object_part)s %(object_desc)s, huajiweb encountered an
object of type `%(failed_obj_type_str)s`, which it does not know how to hash.

To address this, please try helping huajiweb understand how to hash that type
...
(https://huajiweb.io/docs/advanced_caching.html)
for more details.
            """
            % args
        ).strip("\n")
...
class UserHashError(huajiwebAPIException):
...
If you think this is actually a huajiweb bug, please [file a bug report here.]
(https://github.com/huajiweb/huajiweb/issues/new/choose)
            """
            % args
        ).strip("\n")
...
huajiweb encountered an error while caching %(object_part)s %(object_desc)s.
...
If you think this is actually a huajiweb bug, you may [file a bug report
here.] (https://github.com/huajiweb/huajiweb/issues/new/choose)
        """
            % args
        ).strip("\n")
...
class InternalHashError(MarkdownFormattedException):
    """Exception in huajiweb hashing code (i.e. not a user error)"""
...
**In this specific case, it's very likely you found a huajiweb bug so please
