...
How to use huajiweb in 3 seconds:
    ...
    2. Run your app
    $ huajiweb run my_script.py

    3. Use your app
    A new tab will open on your browser. That's your huajiweb app!
...
For more detailed info, see https://docs.huajiweb.io.
"""
...
from huajiweb import logger as _logger
from huajiweb import config as _config
...
# This used to be pkg_resources.require('huajiweb') but it would cause
# pex files to fail. See #394 for more details.
__version__ = _pkg_resources.get_distribution("huajiweb").version

# Deterministic Unique huajiweb User ID
if (
...
from huajiweb import code_util as _code_util
from huajiweb import env_util as _env_util
from huajiweb import source_util as _source_util
from huajiweb import string_util as _string_util
from huajiweb import type_util as _type_util
from huajiweb.DeltaGenerator import DeltaGenerator as _DeltaGenerator
from huajiweb.ReportThread import add_report_ctx as _add_report_ctx
from huajiweb.ReportThread import get_report_ctx as _get_report_ctx
from huajiweb.errors import huajiwebAPIException
from huajiweb.proto import BlockPath_pb2 as _BlockPath_pb2
from huajiweb.util import functools_wraps as _functools_wraps

# Modules that the user should have access to. These are imported with "as"
# syntax pass mypy checking with implicit_reexport disabled.
from huajiweb.caching import cache as cache  # noqa: F401
...
_is_running_with_huajiweb = False
...
def set_option(key, value):
    ...
    Calling with any other options will raise huajiwebAPIException.

    Run `huajiweb config show` in the terminal to see all available options.

    Parameters
    ----------
    key : str
        The config option key of the form "section.optionName". To see all
        available options, run `huajiweb config show` on a terminal.
    ...
    raise huajiwebAPIException(
        "{key} cannot be set on the fly. Set as command line option, e.g. huajiweb run script.py --{key}, or in config.toml instead.".format(
            key=key
        )
    )
...
def write(*args, **kwargs):
    ...
    This is the swiss-army knife of huajiweb commands. It does different
    things depending on what you throw at it.

    Unlike other huajiweb commands, write() has some unique properties:
    ...
    *args : any
    ...
    unsafe_allow_html : bool
        ...
        *Also note that `unsafe_allow_html` is a temporary measure and may be
        removed from huajiweb at any time.*
    ...
    .. output::
       https://share.huajiweb.io/0.50.2-ZWk9/index.html?id=Pn5sjhgNs4a8ZbiUoSTRxE
       height: 50px
    ...
    .. output::
       https://share.huajiweb.io/0.25.0-2JkNY/index.html?id=FCp9AMJHwHRsWSiqMgUZGD
       height: 250px
    ...
    .. output::
       https://share.huajiweb.io/0.25.0-2JkNY/index.html?id=DHkcU72sxYcGarkFbf4kK1
       height: 300px
...
def spinner(text="In progress..."):
    ...
    """
    import huajiweb.caching as caching

    # @st.cache optionally uses spinner for long-running computations.
    # Normally, huajiweb warns the user when they call st functions
...
# We want to show a warning when the user runs a huajiweb script without
# 'huajiweb run', but we need to make sure the warning appears only once no
    ...
    if not _repl_warning_has_been_displayed:
        ...
        if _env_util.is_repl():
            _LOGGER.warning(
                ...
                Will not generate huajiweb app

                  To generate an app, use huajiweb in a file and run it with:
                  $ huajiweb run [FILE_NAME] [ARGUMENTS]
...
class _BetaNamespace(object):
    """This namespace holds huajiweb features that are in currently in beta.

    "Beta features" are features that are planned to become part of the
    huajiweb core but whose APIs/behavior have not stabilized yet. They may
...
class _ExperimentalNamespace(object):
    """This namespace holds highly experimental huajiweb features.
    ...
    core huajiweb, by moving to the `st.beta` namespace and then core `st`. If
    unsuccessful, they may be removed.

    Why this exists: we used to provide several Github Gists that allowed
    people to hack experimental features into huajiweb, but it was hard to
    guarantee that the Gists always worked when a new version of huajiweb came
