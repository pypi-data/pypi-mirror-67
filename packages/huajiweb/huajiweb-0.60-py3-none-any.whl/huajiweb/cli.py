...
"""This is a script which is run when the huajiweb package is executed."""
...
import huajiweb
from huajiweb.credentials import Credentials, check_credentials
from huajiweb import version
import huajiweb.bootstrap as bootstrap
from huajiweb.case_converters import to_snake_case
...
NEW_VERSION_TEXT = """
  ...
  See what's new at https://discuss.huajiweb.io/c/announcements

  Enter the following command to upgrade:
  %(prompt)s %(command)s
""" % {
    "new_version": click.style(
        "A new version of huajiweb is available.", fg="blue", bold=True
    ),
    "prompt": click.style("$", fg="blue"),
    "command": click.style("pip install huajiweb --upgrade", bold=True),
}
...
def _convert_config_option_to_click_option(config_option):
    ...
    envvar = "huajiweb_{}".format(to_snake_case(param).upper())
...
def _apply_config_options_from_cli(kwargs):
    """The "huajiweb run" command supports passing huajiweb's config options
    as flags.
...
@click.group(context_settings={"auto_envvar_prefix": "huajiweb"})
@click.option("--log_level", show_default=True, type=click.Choice(LOG_LEVELS))
@click.version_option(prog_name="huajiweb")
@click.pass_context
def main(ctx, log_level="info"):
    ...
    Or use the line below to run your own script:
        ...
        $ huajiweb run your_script.py
    """

    if log_level:
        import huajiweb.logger

        huajiweb.logger.set_log_level(log_level.upper())
...
def help(ctx):
    ...
    # Pretend user typed 'huajiweb --help' instead of 'huajiweb help'.
    import sys
...
def main_version(ctx):
    """Print huajiweb's version number."""
    # Pretend user typed 'huajiweb --version' instead of 'huajiweb version'
    import sys
...
def main_docs():
    ...
    util.open_browser("https://docs.huajiweb.io")
...
def main_hello(**kwargs):
    ...
    from huajiweb.hello import hello

    _apply_config_options_from_cli(kwargs)

    filename = hello.__file__

    # For Python 2 when huajiweb is actually installed (make install rather
    # than make develop).
...
@main.command("run")
@configurator_options
@click.argument("target", required=True, envvar="huajiweb_RUN_TARGET")
@click.argument("args", nargs=-1)
def main_run(target, args=None, **kwargs):
    """Run a Python script, piping stderr to huajiweb.

    The script can be local or it can be an url. In the latter case, huajiweb
    ...
    if extension[1:] not in ACCEPTED_FILE_EXTENSIONS:
        raise click.BadArgumentUsage(
            "huajiweb requires raw Python (.py) files, not %s.\nFor more information, please see https://docs.huajiweb.io"
            % extension
        )

    if url(target):
        from huajiweb.temporary_directory import TemporaryDirectory

        with TemporaryDirectory() as temp_dir:
            from urllib.parse import urlparse
            from huajiweb import url_util
...
def _main_run(file, args=[]):
    ...
    # Set a global flag indicating that we're "within" huajiweb.
    huajiweb._is_running_with_huajiweb = True

    # Check credentials.
    check_credentials()

    # Notify if huajiweb is out of date.
...
@main.group("cache")
def cache():
    """Manage the huajiweb cache."""
    pass


@cache.command("clear")
def cache_clear():
    """Clear the huajiweb on-disk cache."""
    import huajiweb.caching

    result = huajiweb.caching.clear_cache()
    cache_path = huajiweb.caching.get_cache_path()
    if result:
...
@main.group("config")
def config():
    """Manage huajiweb's config settings."""
    pass
...
def config_show(**kwargs):
    """Show all of huajiweb's config settings."""
...
@click.pass_context
def activate(ctx):
    """Activate huajiweb by entering your email."""
