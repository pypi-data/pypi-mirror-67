...
def huajiweb_read(path, binary=False):
    """Opens a context to read this file relative to the huajiweb path.

    For example:

    with huajiweb_read('foo.txt') as foo:
        ...
    ...
    """ % CONFIG_FOLDER_NAME
    filename = get_huajiweb_file_path(path)
...
def huajiweb_write(path, binary=False):
    ...
    if binary:
        mode += "b"
    path = get_huajiweb_file_path(path)
...
def get_huajiweb_file_path(*filepath):
...
def get_project_huajiweb_file_path(*filepath):
