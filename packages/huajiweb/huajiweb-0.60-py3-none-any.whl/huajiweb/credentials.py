...
"""Manage the user's huajiweb credentials."""
...
EMAIL_PROMPT = """
  ...
  %(email)s""" % {
    "welcome": click.style("Welcome to huajiweb!", bold=True),
    "email": click.style("Email: ", fg="blue"),
}

TELEMETRY_TEXT = """
  %(telemetry)s As an open source project, we collect usage statistics.
  We cannot see and do not store information contained in huajiweb apps.

  If you'd like to opt out, add the following to ~/.huajiweb/config.toml,
...
""" % {
    ...
    "hello": click.style("huajiweb hello", bold=True),
}
...
class Credentials(object):
    ...
    def load(self, auto_resolve=False) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                'Credentials not found. Please run "huajiweb activate".'
            )
            ...
            raise Exception(
                ...
                Run "huajiweb reset" and try again.
                """
                )
                % (self._conf_file)
            )
    ...
    def _check_activated(self, auto_resolve=True):
        """Check if huajiweb is activated.

        Used by `huajiweb run script.py`
        """
        try:
    ...
    def reset(cls):
        ...
        This is used by `huajiweb activate reset` in case a user wants
        to start over.
        """
        ...
        if self.activation:
            if self.activation.is_valid:
                ...
                _exit(
                    ...
                    "`huajiweb activate reset` then `huajiweb activate`"
                )
        else:
            activated = False
...
def _get_credential_file_path():
    return file_util.get_huajiweb_file_path("credentials.toml")
