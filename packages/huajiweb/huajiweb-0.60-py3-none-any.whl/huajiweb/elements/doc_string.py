...
CONFUSING_huajiweb_MODULES = ("huajiweb.DeltaGenerator", "huajiweb.caching")

CONFUSING_huajiweb_SIG_PREFIXES = ("(element, ",)
...
def marshall(proto, obj):
    ...
    if module_name in CONFUSING_huajiweb_MODULES:
        proto.doc_string.module = "huajiweb"
...
def _get_signature(f):
    is_delta_gen = False
    try:
        is_delta_gen = f.__module__ == "huajiweb.DeltaGenerator"
    ...
    if is_delta_gen:
        for prefix in CONFUSING_huajiweb_SIG_PREFIXES:
