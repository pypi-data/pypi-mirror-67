...
from huajiweb.error_util import get_nonhuajiweb_traceback
from huajiweb.errors import MarkdownFormattedException
...
def marshall(exception_proto, exception):
    ...
    """
    # If this is a huajiwebAPIException, we prune all huajiweb entries
    # from the exception's stack trace.
    is_api_exception = isinstance(exception, huajiwebAPIException)
    is_markdown_exception = isinstance(exception, MarkdownFormattedException)

    stack_trace = _get_stack_trace_str_list(
        exception, strip_huajiweb_stack_entries=is_api_exception
    )
...
huajiweb was unable to parse the data from an exception in the user's script.
...
def _get_stack_trace_str_list(exception, strip_huajiweb_stack_entries=False):
    ...
    strip_huajiweb_stack_entries : bool
        If True, all traceback entries that are in the huajiweb package
        will be removed from the list. We do this for exceptions that result
        from incorrect usage of huajiweb APIs, so that the user doesn't see
    ...
    if isinstance(exception, huajiwebAPIWarning):
    ...
    if extracted_traceback is None:
        ...
        if strip_huajiweb_stack_entries:
            extracted_frames = get_nonhuajiweb_traceback(extracted_traceback)
