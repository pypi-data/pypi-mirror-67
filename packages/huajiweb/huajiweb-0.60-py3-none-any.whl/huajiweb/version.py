...
PYPI_huajiweb_URL = "https://pypi.org/pypi/huajiweb/json"
...
def _get_installed_huajiweb_version():
...
def _get_latest_huajiweb_version(timeout=None):
    ...
    """
    rsp = requests.get(PYPI_huajiweb_URL, timeout=timeout)
    try:
...
def should_show_new_version_notice():
    ...
    try:
        installed_version = _get_installed_huajiweb_version()
        latest_version = _get_latest_huajiweb_version(timeout=1)
