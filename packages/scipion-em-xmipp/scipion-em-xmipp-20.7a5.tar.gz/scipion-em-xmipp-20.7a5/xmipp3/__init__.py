...
class Plugin(pwem.Plugin):
    ...
    def defineBinaries(cls, env):
        ...
        def getHome(*path):
            """ cls.getHome() is not defined before registering the plugin.
                This happens when installing plugin in production
                i.e. 'scipion3 installp -p scipion-em-xmipp' then xmippSrc is
                     tried to be installed by default.
            """
            newpath = cls.getHome(*path)
            home = pwem.Config.XMIPP_HOME  # this is always defined
            return newpath if newpath else os.path.join(home, *path)

        # Installation vars for commands formating
        verToken = getHome('v%s' % _currentVersion)
        confToken = getHome("xmipp.conf")
        installVars = {'installedToken': "installation_finished",
                       'bindingsToken': "bindings_linked",
                       'verToken': verToken,
                       'nProcessors': env.getProcessors(),
                       'xmippHome': getHome(),
                       'bindingsSrc': getHome('bindings', 'python', '*'),
                       'bindingsDst': Config.getBindingsFolder(),
                       'xmippLib': getHome('lib', 'libXmipp.so'),
                       'coreLib': getHome('lib', 'libXmippCore.so'),
        ...
        installCmd = ("cd {cwd} && {configCmd} && {compileCmd} N={nProcessors:d} && "
                      "ln -srf {cwd}/build {xmippHome} && cd - && "
                      "touch {installedToken} && rm {bindingsToken} 2> /dev/null")

        installTgt = [getHome('bin', 'xmipp_reconstruct_significant'),
                      getHome("lib/libXmippJNI.so"),
        ...
        sourceTgt = [getHome('xmipp.bashrc')]  # Target for xmippSrc and xmippDev
