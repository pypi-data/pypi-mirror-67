...
def calculatecounts(starttime,endtime,cts):
                    ...
                    Sess.auth = (storagetwitter.username,storagetwitter.password)
                    print(Sess)

                    final_dict = dict()
                    shortcodelst = dict()

                    #starttime = "1398-12-21"
                    #endtime = "1398-12-22"

                    temp_post = "select content_text,id,publish_id from socialmedia_v5 where source = 'twitter-tweets' and ( shdate>='{}' and shdate<='{}')".format(starttime,endtime)
                    print(temp_post)
                    ...
                    count = 0
                    while flag:
                      post_table_temp=results_post['rows'][:]
                      print('check for equal ids')
                      for index,item in enumerate(post_table_temp):
                        if item[1][2:]==item[2]:
                            listtest.append(item)
  
                      #print('1000 chunk added')


                      #print('FIRST SHAPE IS :',len(post_table))
                      '''
                    ...
                    print ("{} records found for post".format(len(listtest)))
                    #print(listtest[0:2])
                    #dftw=pd.read_sql_query(query,db)
                    '''
                    listtest=[]
                    for index,item in enumerate(post_table):
                        if item[1][2:]==item[2]:
                            listtest.add(item)
                            print(index,' rows proceed')



                    '''
                    #print(len(listtest))
                    dftw=pd.DataFrame(listtest,columns=['txt','id','publish_id'])
...
def main(cmd,start,end,c):
    ...
    edgelist_to_pajek('output.txt',
                      output_filename ='outtwitter.net',
                      directed =False,
                      weighted =True,
                      buffer_size =10000)
