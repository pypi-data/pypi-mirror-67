...
class ColumnSelectionBuilder:
    ...
    def build_rule_set_from_list(cls, list_data):
        # TODO: add support for list rules.
        raise NotImplementedError("Currently rule list is not supported.")

    @classmethod
    def build_rule_set_from_dict(cls, dict_data):
        exclude_key = ColumnSelectionRule.IS_EXCLUDE
        builder = cls()
        is_exclude = exclude_key in dict_data and dict_data.pop(exclude_key)
        if is_exclude:
            builder = builder.include_all()
        if len(dict_data) > 1:
            raise ValueError("Invalid input dict data, only one rule could be in the dict.")
        for k, (include_method, exclude_method) in cls.KEY2METHODS.items():
            method = exclude_method if is_exclude else include_method
            if k in dict_data:
                return method(builder, *dict_data[k]).build_rule_set()
