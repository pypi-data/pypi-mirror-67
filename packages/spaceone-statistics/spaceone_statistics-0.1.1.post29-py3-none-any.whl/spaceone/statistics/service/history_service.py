...
class HistoryService(BaseService):
    ...
    def create(self, params):
        ...
        if len(join) > 0 or len(formulas) > 0:
            params['results'] = self.resource_mgr.join_and_execute_formula(results, resource_type, query,
                                                                           join, formulas, sort, page,
                                                                           limit, domain_id)
    ...
    def diff(self, params):
        ...
        to_data = self._get_history_data(topic, diff_filter[:], diff_filter_or[:], diff_to, domain_id)
    ...
    def _get_history_data(self, topic, diff_filter, diff_filter_or, diff_datetime, domain_id):
        query = self._make_statistics_query(topic, diff_filter, diff_filter_or, diff_datetime, domain_id)
    ...
    @staticmethod
    def _make_statistics_query(topic, diff_filter, diff_filter_or, diff_datetime, domain_id):
        diff_filter += [{
            'k': 'topic',
            'v': topic,
            'o': 'eq'
