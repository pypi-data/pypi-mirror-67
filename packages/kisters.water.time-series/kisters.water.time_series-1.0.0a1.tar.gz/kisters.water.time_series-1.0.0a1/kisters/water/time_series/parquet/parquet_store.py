    ...
    def _load_table(self) -> None:
        ...
        for (path, t0, member, dispatch_info), group in master_df.groupby(
            ...
            df_ts = (
                ...
                .set_index(self.time_stamp_column)
            )

            if path in self._time_series:
                self._time_series[path]._data[(t0, member, dispatch_info)] = df_ts
            else:
                self._time_series[path] = ParquetTimeSeries(
                    self,
                    path,
                    master_metadata[path],
                    {EnsembleMemberInfo(t0, member, dispatch_info): df_ts},
                )
