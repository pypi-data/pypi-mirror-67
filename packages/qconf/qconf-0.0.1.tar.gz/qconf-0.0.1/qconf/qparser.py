...
def parse_yaml_json(data):
    ...
    try:
        ...
        json_exc = e
    try:
        d = yaml.safe_load(StringIO(data))
    except Exception as e:
        yaml_exc = e
    if json_exc and yaml_exc:
        raise ParsingException("Error parsing config from data", [json_exc, yaml_exc])
    elif not isinstance(d, Mapping):
        raise ParsingException("Data is parseable but cannot be mapped into a dictionary")
    return d
...
def parse_body(request):
    ...
    if not data:
        raise ParsingException("No data was found in the request")
    return data
