...
from fa_common import create_app, start_app, utils, force_sync, FileDownloadRef
...
async def test_job_create(ensure_test_gitlab):
    ...
    job_run = await run_job(
        project, "Sync Unit Test", module, job_data, files, sync=True
    )
...
async def test_job_create_async(ensure_test_gitlab):
    ...
    job_run = await run_job(project, "Async Unit Test", module, job_data, files)
