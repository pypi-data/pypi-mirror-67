...
for i in range(num_modeling_inits): 
    ...
    dobjc, doptc, ddumh, ddumu, saver = TFV2SimpleTCModelDNN(gs_drm[i], D_tre, D_unt, o_tre, o_unt, c_tre, c_unt, 'train-first-drm', num_hidden) 
    dsess = tf.compat.v1.Session(graph = gs_drm[i]) 
    ...
    if reload: 
        ...
        with gs_drm[i].as_default() as g: 
            dinit = tf.compat.v1.global_variables_initializer() 
        dsess.run(dinit) 
        for step in range(num_optimize_iterations): 
            ...
            if step % 100 == 0: 
                ...
                if step != 0: 
                    ...
                    #print(tf.executing_eagerly())
                    saver.save(dsess, ckpt_pathfile, step)                
    
    print('---> optimization finished ... ') 
    
    ### evaluate CPIT metric on validation set 
    dobjv, ddumo, dumh, dumhu, dsaver = TFV2SimpleTCModelDNN(gs_drm[i], Dv_tre, Dv_unt, ov_tre, ov_unt, cv_tre, cv_unt, 'eval', num_hidden)
