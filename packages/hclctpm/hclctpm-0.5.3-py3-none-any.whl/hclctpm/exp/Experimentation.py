...
class Experimentation: 
    ...
    def plot_lift_curve_random(self, constant_value, num_segments = 10):
        ips = np.reshape(np.arange(0.0, 1.0 + 1.0 / num_segments, 1.0 / num_segments), (-1, 1))
        ios = np.ones((ips.shape[0], ips.shape[1])) * 1.0 * constant_value
        plt.plot(ips, ios)

    def plot_cost_curve_random(self, num_segments=10): 
        ips = np.reshape(np.arange(0.0, 1.0 + 1.0 / num_segments, 1.0 / num_segments), (-1, 1)) 
        ios = np.reshape(np.arange(0.0, 1.0 + 1.0 / num_segments, 1.0 / num_segments), (-1, 1))
        plt.plot(ips, ios) 
    
    def AUC_cpit_cost_curve_deciles_cohort(self, pred_values, values, w, n9d_ni_usd, color=None, plot_random=False, verbose=False): 
        ## function plots the curve for cost vs gain, for model evaluation, the slope of the curve correspond to 
        ## inverse of efficiency ratio as a growth metric 
        
        ## Inputs: 
        ##   pred_values: numpy array of model predictions, shape = (num_samples,) same vertical dimensions as # data points 
        ##   values: actual labels (reward or value) as float 
        ##   w: treatment labels {1,0} as float 
        ##   negcost: numpy array of negative of cost (-1.0 * cost) per session or per user
        ##   color: [Deprecated, default to None] 'r', 'b', or other color specs for pyplot.plot, as str
        ##   plot_random: boolean whether or not plot 'random', a straght line, default to False 
        ##   verbose: boolean whether or not print out cohort level metrics for  debugging, default to False 
        
        ## Outputs:
        ##   AUC: area under curve
        ##   array_c: array of incremental costs, one per segment/decile, for plotting (MLvis)
        ##   array_o: array of incremental gain/order, one per segment/decile, for plotting (MLvis)
        ...
        return aucc, percs, combined_series[:, 2], combined_series[:, 1] 
    
    def plot_cost_curve(self, o_series, c_series, color):
        ### plotting with pyplot 
        plt.plot(o_series, c_series, '-o'+color, markersize=12, linewidth=3)         
        plt.xlabel('Incremental cost % of maximum')
        plt.ylabel('Incremental value % of maximum')
        
        ## manipulate axis to show percentage 
        ax = plt.gca() 
        vals = ax.get_xticks()
        ax.set_xticklabels(['{:,.1%}'.format(x) for x in vals])
        vals = ax.get_yticks()
    ...
    def plot_lift_curve(self, percs, o_series, color, name='value'): 
        ### plotting with pyplot 
        plt.plot(percs, o_series, '-o'+color, markersize=12, linewidth=3) 
        plt.xlabel('% of cohort treated') 
        plt.ylabel('Incremental ' + name + ', % cohort ATE') 
    ...
    def generate_plot(self, series_xs, series_ys, dict_auccs, plot_random='None', plot_name='lift-curve', yaxis='value'): 
        colors = ['r', 'b', 'c', 'g', 'k', 'y', 'm'] 
        plt.figure(); ci = 0

        for key in series_ys:
            first_key = key; break
        
        if plot_random is not 'None':
            if plot_random == 'cost-curve':
                self.plot_cost_curve_random()
            elif plot_random == 'lift-curve':
                self.plot_lift_curve_random(series_ys[first_key][-1]) 
        
        for key in series_xs:            
            if plot_name == 'lift-curve': 
                self.plot_lift_curve(series_xs[key], series_ys[key], colors[ci], name=yaxis)
            elif plot_name == 'cost-curve':
                self.plot_cost_curve(series_xs[key], series_ys[key], colors[ci])
