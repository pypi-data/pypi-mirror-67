...
class Callback(object):
    ...
    def add_storage_output(self, storage, key=None, copy=False):
        ...
        storage.register_callback(callback, key)
        return callback

    def use_storage_inputs(self, storage, *keys, copy=False):
        if copy:
            callback = deepcopy(self)
        else:
            callback = self
        self.dash_inputs = [Input(storage.id, "value")]
        storage.add_deserializer(callback, *keys)
        return callback
