...
class Storage(Component):
    ...
    def layout(self):
        return Div(datetime.now(), id=self.id)
    ...
    def _add_store_prehook(self, callback):
        def pre_hook(_, client_id, *triggers_and_states):
            ...
            return self._deserializer(client_id, trigger_keys, states)
    ...
    def _before_registry(self, app):
        ...
        for provider in self.providers:
            provider.prepend_input(("main-div", "key"))
...
class PickleStorage(Storage):
    ...
    def _serializer(self, cleint_id, callback_id, *outputs):
        if len(outputs) == 1:
            outputs = outputs[0]
...
class ContextStorage(Storage):
    ...
    def _deserializer(self, client_id, trigger_names, _):
        store = self.get_store(client_id)
        return {name: store[name] for name in trigger_names}
