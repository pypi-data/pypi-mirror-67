...
class Earth(object):
	...
	def average_collection(self, image_collection, label = "DATA", band_select = None, reducer = ee.Reducer.median()):
		if band_select:
			image = image_collection.select(band_select).reduce(reducer).rename(label)
		else:
	...
	def mask_by_value(self, image, value):
		mask = image.eq(-9999);
