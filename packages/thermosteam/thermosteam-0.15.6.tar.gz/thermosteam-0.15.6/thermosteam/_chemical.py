    ...
    **data : float or str
    ...
    def reset(self, CAS, eos=PR, phase_ref=None,
        ...
        try:
            ...
            if formula and info.formula and formula != info.formula:
                raise RuntimeError(f'{self.ID} (CAS: {CAS}) formula from database does not '
                                   'match with user data')
            if MW and info.MW and MW != info.MW:
                raise RuntimeError(f'{self.ID} (CAS: {CAS}) molecular weight from database does not '
                                   'match with user data')
    ...
    def _init_data(self, CAS, MW, Tm, Tb, Tc, Pc,
        ...
        # Energy
        self._Hfus = Hfus if Hfus is not None else heat_of_fusion(CAS)
