...
def check_material_feasibility(material: np.ndarray):
    if (material < 0.).any(): raise InfeasibleRegion('reaction conversion')
...
class Reaction:
    ...
    def __call__(self, material, check_feasibility=True):
        material += material[self._X_index]*self.X*self._stoichiometry
        if check_feasibility: check_material_feasibility(material)
