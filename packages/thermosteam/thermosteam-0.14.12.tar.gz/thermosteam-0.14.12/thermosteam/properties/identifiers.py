...
class ChemicalMetadataDB:
    ...
    def _search_autoload(self, identifier, index):
        if index:
            if identifier in index:
                return index[identifier]
            else:
                if self.can_autoload:
                    self.autoload_next()
                    return self._search_autoload(identifier, index)
        return False
    
    def search_pubchem(self, pubchem):
        if type(pubchem) != int:
            pubchem = int(pubchem)
        return self._search_autoload(pubchem, self.pubchem_index)
        
    def search_CAS(self, CAS):
        if type(CAS) != int:
            CAS = CAS2int(CAS)
        return self._search_autoload(CAS, self.CAS_index)

    def search_smiles(self, smiles):
        return self._search_autoload(smiles, self.smiles_index)

    def search_InChI(self, InChI):
        return self._search_autoload(InChI, self.InChI_index)

    def search_InChI_key(self, InChI_key):
        return self._search_autoload(InChI_key, self.InChI_key_index)

    def search_name(self, name):
        return self._search_autoload(name, self.name_index)
    
    def search_formula(self, formula):
        return self._search_autoload(formula, self.formula_index)
