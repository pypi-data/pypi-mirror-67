...
class Chemical:
    ...
    **data : float or str
    ...
    def __new__(cls, ID, *, search_ID=None,
                eos=PR, phase_ref=None,
                default=False, phase=None,
                CAS=None, search_db=True,
                V=None, Cn=None, mu=None,
                sigma=None, kappa=None, epsilon=None,
                **data):
        cache = cls._cache
        search_ID = search_ID or CAS or ID
        try:
            ...
            try: CAS = CAS_from_any(search_ID)
                ...
                if search_db: raise Error
        if search_db and CAS in cache:
            self = cache[CAS]
            if ID != self._ID:
                if ID in cache:
                    self = cache[ID]
