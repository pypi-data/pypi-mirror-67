...
class Monitaur:
    ...
    def record_training_tabular(
        ...
        with open(filename, "rb") as f:
            joblib_data = (base64.b64encode(f.read())).decode("utf-8")
        with open(filename_anchors, "rb") as f:
            anchor_data = (base64.b64encode(f.read())).decode("utf-8")

        # Send Joblib and Anchor data to api for s3 upload and Update Training Version
        payload = {
            "model_set_id": model_set_id,
            "version": version,
            "model_name": f"{model_set_id}/{version}/{filename}",
            "model_data": joblib_data,
            "anchor_name": f"{model_set_id}/{version}/{filename_anchors}",
