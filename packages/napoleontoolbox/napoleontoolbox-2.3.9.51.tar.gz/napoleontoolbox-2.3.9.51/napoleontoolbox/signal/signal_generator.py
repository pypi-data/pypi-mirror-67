...
def select_orthogonal_coef(wtmra, dec_level, coeff_index_factor):
    components = wtmra.copy()
    ortho_coeff = np.zeros(components.shape)
    for j in range(len(components)):
        ...
        ortho_coeff[j] = current_array * components[j]
    return sum(ortho_coeff)
...
def fourier_wvt_ortho_decompo_lo(data=None, filter_name='haar', dec_level=2, n_harm=5,
                                 fft_filter_type='low', coeff_index_factor=2, **kwargs):
    n_predict = 1
    try:
        x = data['return'].values
        wt = wavelet_features.modwt(x, filter_name, dec_level)
        decomp_components = wavelet_features.modwtmra(wt, filter_name)
        approx = select_orthogonal_coef(decomp_components, dec_level, coeff_index_factor)
        lb_signal = fourier_extrapolation(approx, n_predict, n_harm, fft_filter_type)
...
def fourier_wvt_ortho_decompo_ls(data=None, filter_name='haar', dec_level=2, n_harm=5, fft_filter_type='low', coeff_index_factor=2, **kwargs):
    n_predict = 1
    try:
        x = data['return'].values
        wt = wavelet_features.modwt(x, filter_name, dec_level)
