...
def f_sharpe(assets, X, previous_w, cost_parameters, w):
    ...
    transaction_costs =np.dot(abs(w - previous_w) , cost_parameters)
    w = w.reshape([N_, 1])
    to_minimize = - metrics.sharpe(np.cumprod(np.prod(X[:, assets] @ w + 1, axis=1)))
    #return to_minimize
    return to_minimize + transaction_costs
...
def f_calmar(assets, X, previous_w, cost_parameters, w):
    ...
    transaction_costs =np.dot(abs(w - previous_w) , cost_parameters)
    w = w.reshape([N_, 1])
    to_minimize = - metrics.calmar(np.cumprod(np.prod(X[:, assets] @ w + 1, axis=1)))
    #return to_minimize
    return to_minimize + transaction_costs
            ...
            if t + rebal <= T:
                ...
                if N_ != 0:
                    ...
                    for f in f_list:
                        ...
                        func_to_optimize = partial(eval(f), assets, X, previous_w_, costs_parameters)
                        # Optimize f
                        w__ = minimize(
                            func_to_optimize,
                            previous_w_[assets],
