...
class ThreadLocalMiddleware(MiddlewareMixin):
            ...
            if request.body and request.content_type == 'application/json':
                ...
                if 'message' in data:
                    if 'messageId' in data['message']:
                        ...
                        if 'child_document_id' in published_data:
                            child_document_id = published_data['child_document_id']
                            setattr(local, 'child_document_id', child_document_id)
    ...
    def process_response(self, request, response):
        ...
        setattr(local, 'child_document_id', None)
        return response
...
class ChildDocumentIdFilter(logging.Filter):
    def filter(self, record):
        record.child_document_id = getattr(local, 'child_document_id', None)
        return True