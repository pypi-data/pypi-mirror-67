...
class AssertToken(object):
    ...
    def is_comparison(self):
        ...
        """
        return isinstance(self.token.test, ast.Compare) and len(self.token.test.ops) == 1

    @property
    def is_name_or_attribute_or_call(self):
        return isinstance(self.token.test, (ast.Name, ast.Attribute, ast.Call))

    @property
    def is_not(self):
        return isinstance(self.token.test, ast.UnaryOp) and isinstance(self.token.test.op, ast.Not)
    ...
    def has_replacement(self):
        return self.is_name_or_attribute_or_call or self.is_not or self.comparitor in ASSERT_REPLACEMENTS
    ...
    def replacement(self):
        ...
        return ASSERT_CALL_TMPL.format(indent=self.indent, function=self.function_used, args=", ".join(args))
...
def comment(text):
    comments = [t[1] for t in tokenize.generate_tokens(StringIO(text).readline) if t[0] == tokenize.COMMENT]
        ...
        if python_line in line_replacements:
            ...
            if a.function_used:
                ...
                try:
                    ...
                    LOG.warning("If lines {}-{} have comments, they will be lost".format(a.linenum, a.linenum + a.extra_lines))
            skip_lines = a.extra_lines
        else:
            fixed.append(contents)

    # Add an import statement at the top of the file (this may need fixing)
    import_line = "from assertions import {}".format(", ".join(f for f in functions))
