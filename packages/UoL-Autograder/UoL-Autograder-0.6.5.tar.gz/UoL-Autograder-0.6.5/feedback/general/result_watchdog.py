...
class ResultWatchdog:
    ...
    def _launch_watchdog(self):
        ...
        observer = Observer()
        observer.schedule(event_handler, path=os.path.dirname(self.result_file), recursive=False)
        observer.start()
        start = time.time()
    ...
    def _write_tamper_alert_result(self):
        util.save_feedback_file(TAMPER_ALERT_RESULT, self.result_file, callback=self.submit_hash)
        

class Handler(PatternMatchingEventHandler):
    def __init__(self, result_file, callback):
        PatternMatchingEventHandler.__init__(self, patterns=[result_file], ignore_directories=True, case_sensitive=False)
