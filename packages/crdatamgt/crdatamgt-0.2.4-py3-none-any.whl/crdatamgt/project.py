...
def project_load_from_workbooks(workbooks, **kwargs):
    ...
    compiled = pd.concat(project_sheets[::-1], sort=False).set_index('Topic ID').sort_index().reset_index()
    ...
    workbook_save("Project Results", output_path, compiled, 'Results', **formatting)
