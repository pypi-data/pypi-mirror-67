...
def _write_formulations_tests(new_data, formulation_path):
    ...
    if not (id_only.empty and with_formulation.empty):
        ...
        used_formulations = updated_data.reindex(formulation_id['Formulation ID'])
        used_formulations = used_formulations.drop(columns=['test', 'Formulation ID'], errors='ignore')
