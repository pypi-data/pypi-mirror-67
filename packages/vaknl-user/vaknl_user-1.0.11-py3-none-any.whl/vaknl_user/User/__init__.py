...
class User(object):
        ...
        if isinstance(event, EventDataclass.ReservationBooked):
            ...
            if isinstance(event, (EventDataclass.ReservationPersonalData, EventDataclass.ReservationOverview,
                ...
                if (
                        ...
                        len(statistics['acco_freq'].keys()) >= 5
                ):
    ...
    def update_statistics(self, event):
        ...
        if self.statistics['event_cnt'] == 0:
            self.statistics['created_at'] = event.timestamp
        else:
            self.statistics['created_at'] = min(self.statistics['created_at'], event.timestamp)

        self.statistics['updated_at'] = max(self.statistics['updated_at'], event.timestamp)
        ...
        if isinstance(event, EventDataclass.Pageview):
            ...
            if isinstance(event, EventDataclass.PageviewProductPage):
                ...
                giataid_counter = self.statistics['acco_freq']
                giataid_counter[giataid] = giataid_counter[giataid] + 1 if giataid in giataid_counter else 1
                self.statistics['acco_freq'] = giataid_counter
                self.statistics['acco_last_viewed'] = giataid
                self.statistics['acco_mode'] = max(giataid_counter, key=giataid_counter.get)
                if giataid:
