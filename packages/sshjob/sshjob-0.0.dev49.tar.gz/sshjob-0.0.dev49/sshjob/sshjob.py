...
class sshjobsys(OrderedDict):
    ...
    def logfiles(self,key):
        ...
        return sshdir,fno,fne

    def sshcat(self,sshdir,filename,tail=20,pipe=None,egrep=None,grep=None,pattern=None,**kwargs):
        # TODO : move this function to shell_run.py with sshdir
        if pipe:
            com="""
             cat %s | %s ;
            """%(filename,pipe)
    ...
    def stdout(self,key,**kwargs):
        sshdir,fno,fne = self.logfiles(key)
        print('*** stdout file: %s/%s'%(sshdir,fno))
        return self.sshcat(sshdir,fno,**kwargs)

    def stderr(self,key,**kwargs):
        sshdir,fno,fne = self.logfiles(key)
        print('*** stderr file: %s/%s'%(sshdir,fne))
        return self.sshcat(sshdir,fne,**kwargs)
