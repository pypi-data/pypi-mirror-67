...
class oauth():
    ...
    def cache_token(self, code):
        # Get token
        response = requests.post('https://untappd.com/oauth/authorize/?client_id=' + self.client_id +
                                '&client_secret=' + self.client_secret +
                                '&response_type=code&redirect_url=' + self.callback_url +
                                '&code=' + code)
        if response:
    ...
    def get_url(self):
        return('https://untappd.com/oauth/authenticate/?client_id=' +
                self.client_id + '&response_type=code&redirect_url=' +
                self.callback_url)
