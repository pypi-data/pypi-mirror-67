...
class Profile:
    ...
    def __init__(
        self, file_paths, parser, q_axis_name="qdcd", theta_axis_name="dcdtheta", pixel_max=1000000
    ):
        self.scans = []
        for f in file_paths:
            self.scans.append(Scan(f, parser, q_axis_name, theta_axis_name, pixel_max=pixel_max))
        self.q_vectors = None
...
class Scan:
    ...
    def crop_and_bkg_sub(
        ...
        to_remove = []
        for i in iterator:
            im = image.Image(self.data["file"][i], self.data, self.metadata, pixel_maximum=self.pixel_max)
            if im.skip:
                to_remove.append(i)
                continue
