...
import os
import cv2
from simpleocr.localization import model as __localization_model
from simpleocr.recognition import model as __recognition_model
...
def get_text(images: list, text_only=False, dist_dir='./output'):
    ...
    for image_path in images:
        ...
        bboxes = __localization_model.get_text_localization(original_image,
        ...
        for index, bbox in enumerate(bboxes):
            ...
            if clipped_image.shape[0] > 1 and clipped_image.shape[1] > 1:
                if not text_only:
                    cv2.imwrite(os.path.join(dist_dir, f'{filename}_{index}.jpg'), clipped_image)

                text = __recognition_model.get_text(clipped_image,
                                                  weights=os.path.join(base_path, 'data', 'weights',
