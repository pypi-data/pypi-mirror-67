...
class VacBot():
    ...
    def _handle_major_map(self, event):
        ...
        # Initialize new map Cache
        if len(self.mapPieces) == 0:
            for i in range(64):
                self.mapPieces.append(str(i) + '-' + str(values[i]))
                self.exc_command('getMinorMap', {'mid': response['mid'],'type': 'ol', 'pieceIndex': i})
        else:
            for i in range(64):
                if self.mapPieces[i] != str(i) + '-' + str(values[i]):
                    self.mapPieces[i] = str(i) + '-' + str(values[i])
                    _LOGGER.debug("UPDATING MAP PIECE: " + str(i))
                    self.exc_command('getMinorMap', {'mid': response['mid'],'type': 'ol', 'pieceIndex': i})
    ...
    def CreatePNGMap(self):
        ...
        for i in range(64):
            ...
            _LOGGER.debug('PIECE: ' + str(i) + ' X:' + str(imageX) + ' Y:' + str(imageY))
