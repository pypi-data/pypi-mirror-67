...
class VacBot():
    def __init__(self, user, domain, resource, secret, vacuum, continent, server_address=None, monitor=False, verify_ssl=True):
        ...
        # Map variables
        self.buffer = np.zeros((1000, 100, 100))
        self.mapPieces = []
    ...
    def _handle_minor_map(self, event):
        ...
        self.AddMapPiece(response['pieceIndex'], response['pieceValue'])
    ...
    def _handle_cached_map(self, event):
        ...
        for i in range(64):
            self.exc_command('getMinorMap', {'mid': mapid,'type': 'ol', 'pieceIndex': i})
    ...
    def AddMapPiece(self,mapPiece, b64):
        ...
        self.UpdateMapBuffer(mapPiece, MATRIX_PIECE)
    ...
    def CreatePNGMap(self):
        im = Image.new("RGB", (6400, 6400))
        draw = ImageDraw.Draw(im)

        imageX = 0
        imageY = 0

        prev = self.mapPieces[0] - 1
        for mapPiece in self.mapPieces:
            if mapPiece != self.mapPieces[0]:
                if mapPiece % 8 != 0:
                        imageY += 100
                else:
                    imageX += 100
            ...
            _LOGGER.debug('PIECE: ' + str(mapPiece) + ' X:' + str(imageX) + ' Y:' + str(imageY))

            for x in range(100):
                for y in range(100):
                    if self.buffer[mapPiece][x][y] == 0x01: #floor
                        draw.point((imageX+x,imageY+y), fill=(186,218,255))
                    if self.buffer[mapPiece][x][y] == 0x02: #wall
        ...
        self.live_map = base64.b64encode(buffered.getvalue())

    def UpdateMapBuffer(self, mapPiece, matrix):
        self.mapPieces.append(mapPiece)
        self.buffer[mapPiece] = matrix
