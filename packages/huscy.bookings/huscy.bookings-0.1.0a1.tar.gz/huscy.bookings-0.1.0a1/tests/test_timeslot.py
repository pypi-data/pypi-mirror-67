...
def test_end_property(setup_time, duration, teardown_time, expected_end):
    session = mommy.prepare('projects.Session', duration=duration,
                            setup_time=setup_time, teardown_time=teardown_time)
    timeslot = mommy.prepare('bookings.Timeslot', session=session, start=datetime(2000, 1, 1, 12))
...
def test_has_overlap_with_booked_timeslots(booked_timeslot, start, duration, location, has_overlap):
    ...
    session = mommy.make('projects.Session', duration=duration)
    mommy.make('projects.DataAcquisitionMethod', session=session, location=location)
    timeslot = mommy.make('bookings.Timeslot', session=session, start=start)
