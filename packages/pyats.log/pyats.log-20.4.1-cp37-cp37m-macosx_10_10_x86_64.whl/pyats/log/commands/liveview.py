...
class LiveViewSIO(object):
    ...
    async def connect_and_subscribe(self, app):
        ...
        loop.create_task(self.reportclient.subscribe(self.print_section))
        loop.create_task(self.get_testsuite_impl())
    ...
    def start(self, host, port):
        try:
            self.p = Process(target=self.serve, kwargs={'host': host, 
                                                        'port': port})
            # run liveview server in a seperate process
            self.p.start()
