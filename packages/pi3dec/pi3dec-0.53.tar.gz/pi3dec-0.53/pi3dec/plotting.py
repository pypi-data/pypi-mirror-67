...
def plot_tube(pe, tube, log_OD=None, log_pumps=None, plot_growth_rate=True,window_size=30):
    ...
    for j in range(3):
        ...
        started_indeces = np.where(pdata.comment.str.contains("started"))[0]

        for i in started_indeces:
            if "completed" in pdata.iloc[i + 1].comment:
                start_time = (pdata.index[i] - time0).total_seconds() / 3600
                end_time = (pdata.index[i + 1] - time0).total_seconds() / 3600
                vol = pdata.iloc[i, 0]
                assert vol == pdata.iloc[i + 1, 0]
                plt.plot([start_time, end_time], [vol, vol], "s-", color=colors[j], markersize=4, linewidth=4,
                         alpha=alpha[j], label=labels[j])  # solid_capstyle="butt"
