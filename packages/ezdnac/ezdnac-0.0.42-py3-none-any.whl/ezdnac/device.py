...
class device():
	...
	def deployTemplate(self, templateId, templateParams):
			...
			if response.status_code == 202:
				...
				# if no id was found, try to see what the problem is.
				resultRegex = (re.findall(r'None of the targets are applicable for the template. Hence not deploying', result))
				if len(resultRegex) is not 0:
					message = result
