...
<%def name="matches_row(rank, row)">
  <tr>
    ...
    % if tboards:
      <%
      misses1 = sum(b.score1 for b in row.boards) if boardcoins1 else None
      misses2 = sum(b.score2 for b in row.boards) if boardcoins2 else None
      %>
      % for i in range(tboards):
        <td class="right aligned">
          ${row.boards[i].score1 if row.boards and row.boards[i].score1 is not None else '—'}
        </td>
      % endfor
      <td class="right aligned">
      ...
      % for i in range(tboards):
        <td class="right aligned">
          ${row.boards[i].score2 if row.boards and row.boards[i].score2 is not None else '—'}
        </td>
      % endfor
      <td class="right aligned">
