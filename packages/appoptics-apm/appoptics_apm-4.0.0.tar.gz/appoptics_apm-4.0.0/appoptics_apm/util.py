...
def _get_logger():
    ...
    if ('APPOPTICS_DEBUG_LEVEL' in os.environ and int(os.environ['APPOPTICS_DEBUG_LEVEL']) == OBOE_DEBUG_DISABLE):
        ...
        _logger.setLevel(_logging_map[int(os.getenv('APPOPTICS_DEBUG_LEVEL', str(OBOE_DEBUG_WARNING)))])
    return _logger
...
class AppOpticsApmConfig(object):
    ...
    def __init__(self, **kwargs):
        ...
        self._config = {
            ...
            'debug_level': OBOE_DEBUG_WARNING,
            'warn_deprecated': True,
            'service_key': '',
            'hostname_alias': '',
            'trustedpath': '',
            'events_flush_interval': -1,
            'events_flush_batch_size': -1,
    ...
    def _set_config_value(self, keys, val):
        ...
        try:
            ...
            logger.warning(
                ...
                '.'.join(keys if keys[0] not in ['inst', 'transaction'] else key[1:]))
...
def appoptics_apm_init():
    ...
    if sys_is_traceable():
        ...
        oboe_log_level = config['debug_level']
        logger.setLevel(_logging_map[int(oboe_log_level)])
...
def _reporter():
    ...
    if not reporter_instance:
        reporter_instance = Reporter(
            ...
            events_flush_batch_size=config.get('events_flush_batch_size'),
