...
class PluginsIndex:
    ...
    def get_user_installed_dlls(self) -> List[Path]:
        ...
        """
        ext = _plugin_extension()
        user_path = self.user_plugins_path
        if user_path is None:
            debug("get installed dlls: user plugins path not set")
            return []
        assert self.is_user_plugins_path_set()
        return list(user_path.glob("*" + ext))
    ...
    def get_plugin_installed_version(self, plugin_name: str) -> Optional[str]:
        ...
        if not info:
            debug("Plugin {plugin_name} is not installed")
            return None
    ...
    for plugin in plugins_index.plugins:
        ...
        if info:
            ...
            if not info.installed_in_system_folder:
                extra_lines.append("Path: {info.path}")
        if data:
            status = "[" + ", ".join(data) + "]"
        else:
            status = ""
