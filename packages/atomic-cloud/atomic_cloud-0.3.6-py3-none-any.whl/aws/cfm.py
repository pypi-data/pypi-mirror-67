    ...
    try:
        while stacks_remain:
            if token and stack_status_filter:
                stacks = _cfn.list_stacks(StackStatusFilter=stack_status_filter, NextToken=token)
            elif stack_status_filter:
                stacks = _cfn.list_stacks(StackStatusFilter=stack_status_filter)
            elif token:
                stacks = _cfn.list_stacks(NextToken=token)
            else:
                stacks = _cfn.list_stacks()
    ...
    try:
        while stacks_remain:
            if token and stack_name:
                stacks = _cfn.describe_stacks(StackName=stack_name, NextToken=token)
            elif stack_name:
                stacks = _cfn.describe_stacks(StackName=stack_name)
            elif token:
                stacks = _cfn.describe_stacks(NextToken=token)
            else:
                stacks = _cfn.describe_stacks()
...
def create_stack(fn: str, params={}, capabilities=[], stack_name: str = None, debug = False, region: str = None):
    """
    ...
    :param region: If provided, create the template in that region. Otherwise uses the default region.
    :return: `cloudformation.Stack <https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/cloudformation.html#stack>`_
    """
    global _cfn
    file = None
    ...
    try:
        ...
        # get params list
        template_validated = _cfn.validate_template(TemplateBody=template_body)
        ...
        # create stack
        metadata = _cfn.create_stack(StackName=stack_name,
