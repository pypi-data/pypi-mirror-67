...
class FP16Optimizer:
    ...
    def initialize_flat_fp32_weight(self):
        ...
        for p in self.fp16_model.parameters():
            nelem += p.numel()
        fp32_params = torch.empty(nelem, dtype=torch.float32, device=torch.device("cuda" if self.use_cuda else "cpu"))
        fp16_params = torch.empty(nelem, dtype=torch.float16, device=torch.device("cuda" if self.use_cuda else "cpu"))
    ...
    def step(self, closure=None):
        ...
        end3 = datetime.now()
        logger.info("Took {} to agg, {} to flatten and {} for rest".format(end - start, end2 - end, end3 - end2))
        return updated
...
class FP32Optimizer:
    ...
    def __init__(self, model, world_size, use_cuda=False, by_layer=False, grad_clip=None, average_models=True):
        """
