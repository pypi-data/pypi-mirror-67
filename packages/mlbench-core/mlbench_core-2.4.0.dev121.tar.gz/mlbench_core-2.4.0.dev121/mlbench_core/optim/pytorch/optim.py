...
class CentralizedFusedAdam(FusedAdam):
    r"""Implements centralized Adam algorithm.
    ...
    def __init__(
        ...
        super(CentralizedFusedAdam, self).__init__(params, lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, amsgrad=amsgrad)
        if average_models:
            self.agg_mode = "avg"
        else:
            raise NotImplementedError("Only average model is supported right now.")
    ...
    def step(self, closure=None, grads=None, output_params=None, scale=None, grad_norms=None):
        ...
        """
        self.agg(self.model, self.agg_mode)
        loss = super(CentralizedFusedAdam, self).step(closure=closure, grads=grads, output_params=output_params, scale=scale, grad_norms=grad_norms)
        return loss
