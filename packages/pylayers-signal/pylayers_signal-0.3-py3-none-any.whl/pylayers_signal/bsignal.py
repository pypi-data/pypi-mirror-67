...
"""
from pylayers_project.project import Project
import doctest
import os
import pdb
import numpy as np
...
import pylayers_signal.plotutil as pltu
            ...
            if iy==-1:
                ...
                for k in range(Nr):
                    for l in range(Nt):
                        for ix in idx:
                            yx = self.y[ix,k,l,u]
                            fig,a = pltu.mulcplot(self.x[u],yx*conversion,fig=fig,ax=ax[k,l],**args)
            else:
                fig = plt.gcf()
                ax = np.array([[plt.gca()]])
                yx = self.y[iy[0],iy[1],iy[2],u]
                fig,a = pltu.mulcplot(self.x[u],yx*conversion,fig=fig,ax=ax[0,0],**args)


        if ndim == 3:
            shy = self.y.shape
            yx = self.y.reshape(shy[0]*shy[1],shy[2])[:,u]
            fig,ax = pltu.mulcplot(self.x[u],yx*conversion,**args)
        if ndim == 2:
            if iy==-1:
                yx = self.y[:,u]
            else:
            ...
            fig,ax = pltu.mulcplot(self.x[u],yx*conversion,**args)
        if ndim == 1:
            fig,ax = pltu.mulcplot(self.x[u],self.y[u]*conversion,**args)

        #
        # Draw vertical and horizontal lines
        #
        # To be added in pyu.mulcplot
        #
...
class FUsignal(FBsignal,Usignal):
    ...
    def symHz(self,Nz,scale='extract'):
        ...
        >>> y1    = np.random.randn(N)+1j*np.random.rand(N)
        >>> y2    = np.random.randn(N)+1j*np.random.rand(N)
        >>> y1[0] = 0
        >>> y2[0] = 0
        >>> y     = np.vstack((y1,y2))
        >>> S     = FUsignal(x,y)
