...
class BaseProcessor(ABC):
    ...
    def _get_cache_path(self, data_path):
        """create cache path according to data_path
        :param data_path: data file path
        :return: cache file path under the same directory with data_path, whose file name is 'cached_{md5}'
        """
...
class BaseModeler(object):
    ...
    def train(self, output_dir, data_loader, num_epochs, **kwargs):
        ...
        for _ in trange(num_epochs):
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                outputs = self._train_batch(batch)
                loss = outputs[0]
                loss.backward()
    ...
    def predict(self, examples, features, **kwargs):
        """ 预测结果，输出DataFrame结果以供预览 """
        raise NotImplementedError
