...
class GdsBlackBox(_BlackBox):
    ...
    def __init__(self, cell: "Cell", mapping=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._cell = cell
        self._layout: "Layout" = cell.layout()
    ...
    @mapping.setter
    def mapping(self, m):
        missing_keys = set(m.keys()) - self.REQUIRED_MAPPING_KEYS
        if missing_keys:
            raise ValueError(
    ...
    def bb_info(self):
        if self._bb_info is None:
            ...
            if i is None:
                raise ValueError("BB info layer is not found in the layout.")
            shapes = self._cell.shapes(i)
            n_data = shapes.size()
            if n_data == 0:
                raise ValueError(
                    f"No BB info items are found ({n_data})."
                )
            elif n_data > 1:
                raise ValueError(
                    f"More than a single BB info items are found ({n_data})."
                )
            shape = list(shapes.each())[0]
    ...
    @property
    def name(self):
        return self.bb_info["cell_name"]

    @property
    def version(self):
        return self.bb_info["bb_version"]
    ...
    def get_pin_infos(self) -> Dict[str, TPinInfo]:
        if self._pin_infos == {}:
            ...
            n_data = shapes.size()
            if n_data == 0:
                raise ValueError(f"No pins are found in the cell.")

            for shape in shapes.each():
                if not shape.is_text():
                    raise ValueError(
                        "Pin info layer contains non-text objects."
                    )
                ...
                if "name" not in pin_info:
                    raise ValueError(
                        f"The pin info {pin_info} does not contain pin name."
                    )
                ...
                if "xya" in pin_info:
                    try:
                        ...
                        raise ValueError(
                            "The pin xya for pin {} must contain a list with "
                            "three numeric values. {} is given.".format(
                                pin_name, pin_info["xya"]
                            )
                        )
                    if not isclose(
                        xt, x, abs_tol=1e-10, rel_tol=1e-10
                    ) or not isclose(yt, y, abs_tol=1e-10, rel_tol=1e-10):
                        raise ValueError(
...
class GdsFile(object):
    ...
    def __init__(self, file_name: str, mapping=None):
        ...
        self._layout = Layout()
        self._layer_map = self._layout.read(file_name)
    ...
    @mapping.setter
    def mapping(self, m):
        missing_keys = self.REQUIRED_MAPPING_KEYS - set(m.keys())
        if missing_keys:
            raise ValueError(
                f"The layer mapping has the following layers missing: "
                f"{missing_keys}."
...
class BbValidator(object):
    ...
    def _find_bb_name_by_cell_name(
        ...
        try:
            ...
            msg = (
                f"Error getting black box name for cell '{cell_name}':" 
                f" {e}"
            )
            return [], 0, msg
