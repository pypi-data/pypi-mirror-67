...
class MagneticConfig(Base):
    ...
    def from_currents(cls,
        ...
        kwargs['coilsIds'] = kwargs.pop('coilsIds', w7x.Defaults.MagneticConfig.coilsIds)
        kwargs['coilsIdsCurrents'] = currents[:5] * 10  # non planar coils
        kwargs['coilsIdsCurrents'] += currents[5:7] * 10  # planar coils
        if len(currents) == 14: # stellarator symmetric sweep coils
            kwargs['coilsIdsCurrents'] += currents[7:9] * 5  # sweep coils
            kwargs['coilsIdsCurrents'] += currents[9:]  # trim coils
        if len(currents[7:]) == 22:
            kwargs['coilsIdsCurrents'] += currents[7:]  # 10 sweep, 2 trim coils
        n_total_expected = 85  # yes, that looks hardcoded but is a constant
        if not len(kwargs['coilsIds']) == len(kwargs['coilsIdsCurrents']) == n_total_expected:
            raise ValueError("#Coils: {0}, \t#Currents: {1}, \texpected: {2}"
                             .format(len(kwargs['coilsIds']),
                                     len(kwargs['coilsIdsCurrents']),
                                     n_total_expected)
                             )
        # TODO: test if removing coilsIds with current 0 gets you speed
    ...
    def from_afs_file(cls, filePath, **kwargs):
        ...
        if not filePath.startswith(w7x.Defaults.Paths.tracer_network_share):
    ...
    def from_dat_file(cls, file_path_dat,
        ...
        if file_path_dat.startswith(network_share):
            log.warning("Passing network share path to 'from_dat_file'."
        ...
        if os.path.exists(local_mount_file_path_dat):
            ...
            log.info(file_already_exists_message)
        elif local_mount_file_path_dat:
            # Copy file to nas
            log.info('Copy {file_path_dat} to {local_mount_file_path_dat}'
                     .format(**locals()))
            ...
            import subprocess
            log.info('Copy {file_path_dat} to {network_file_path_dat}'
                     .format(**locals()))
            ...
            while attempts < n_tries:
                ...
                if p.returncode != 0:
                    log.error(u"Attempting to copy by smbclient failed. "
                              u"Number of attemps: %s (cmd: %s)" % (
...
if __name__ == "__main__":
    import doctest
    # doctest.run_docstring_examples(MagneticConfig.from_dat_file, globals())
    # quit()
    doctest.testmod()
    ...
    # for ci in w7x.Coils.idealNonPlanarCoilsIds:
    #     print ci
