    ...
    def options(parser):
        ...
        parser.add_argument(
            ...
            choices=[
                ...
                "SMB2_DIALECT_3_11",
                ""],
            metavar="{smb2_dialect_2_1,smb2_dialect_3_0,smb2_dialect_3_11}",
            nargs="+",
...
class SMBCloseSessionsCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        parser.add_argument("--identity",
            ...
            help='Close only sessions matching this user\'s identity in the '
                 ...
                 'domain prefix (e.g "local:name", "world:Everyone", '
                 '"ldap_user:name", "ldap_group:name", or "ad:name"), or '
                 '[2] An ID type (e.g. "uid:1001", "gid:2001", '
                 '"auth_id:513", "SID:S-1-1-0").')
        parser.add_argument("--location",
            type=str, default=None, required=False,
    ...
    @staticmethod
    def main(conninfo, credentials, args, smb_mod=smb, auth_mod=auth,
            opts_mod=qumulo.lib.opts):
        ...
        sessions = []
        for f in smb_mod.list_smb_sessions(conninfo, credentials):
        ...
        if args.identity is not None:
            id_in = Identity(args.identity).dictionary()
            found_id = auth_mod.find_identity(conninfo, credentials, **id_in)
            auth_id = found_id.data['auth_id']
            sessions = [i for i in sessions if str(auth_id) == str(i['user'])]
