...
class Mqtt:
    ...
    def __init__(self, config, gateway_config: dict = None, _async: bool = False):
        ...
        self.client = MQTTClient(self.config.client_id,
                                 config={"default_qos": QOS_1, "auto_reconnect": True, "reconnect_max_interval": 60,
                                         "reconnect_retries": -1})
    ...
    async def loop(self):
        while True:
            ...
            try:
                ...
                log.exception(e)
                continue
            handle = self.sub_handles.get(msg.topic)
            if handle:
                try:
                    self.sub_handles[msg.topic](msg)
                except BaseException as e:
                    log.exception(e)

    async def publish(self, topic, payload, qos):
        self.mqtt_log(topic, payload)
    ...
    def mqtt_log(self, topic, payload):
...
class Device:
    ...
    async def _setup(self):
        if not self.is_gateway:
            await self.mqtt.publish(self.base_topic + "attach",
                                    ...
                                    qos=1)
        await self.mqtt.subscribe(self.base_topic + "config", self.handle_config, qos=1)
        await self.mqtt.subscribe(self.base_topic + "errors", self.handle_errors, qos=1)
        await self.mqtt.subscribe(self.base_topic + "commands/#", self.handle_commands, qos=1)

    async def subscribe(self, topic, callback, qos=1):
        """
    ...
    async def send(self, *, data: dict, topic_type: str, **kwargs):
        ...
        await self.mqtt.publish(self.base_topic + "events/telemetry", payload.encode('utf-8'), qos=1)
    ...
    async def publish_telemetry(self, msg):
        ...
        if self.evt_setup.is_set():
            await self.mqtt.publish(self.base_topic + "events/telemetry", payload=msg, qos=1)
    ...
    async def publish_command_response(self, data, timestamp: datetime.datetime):
        ...
        if self.evt_setup.is_set():
            await self.mqtt.publish(self.base_topic + "events/telemetry/commands", payload=json.dumps(data), qos=1)
