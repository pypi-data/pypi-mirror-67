...
class TextField(Field):
    ...
    def _prepare(self, use_pretrained_tokenizer: bool = False) -> None:
        if self.text is None:
            raise ConfigurationError("After tokenization text is deleted for memory efficiency.")
    ...
    def count_vocab_items(self, counter: Dict[str, Dict[str, int]]) -> Dict[str, Dict[str, int]]:
        if self._tokenized_text is None:
            self._prepare()
        if self._tokenized_text is None:
            raise ConfigurationError("You must tokenize text before count_vocab_items")
    ...
    def index(self, vocab: Vocabulary, use_pretrained_tokenizer: bool = False) -> None:
        ...
        if self._tokenized_text is None:
            raise ConfigurationError("You must tokenize text before token indexing")
    ...
    def as_tensor(self, padding_length: Dict[str, int]) -> torch.Tensor:
        if self._indexed_tokens is None:
            raise ConfigurationError("You must index fields before 'as_tensor' applying")
    ...
    def __len__(self) -> int:
        if self.text is None:
            raise ConfigurationError("You can't get `len` after indexing field. Original text was deleted.")
        return len(self.text)

    def __str__(self) -> str:
        if self.text is None:
            raise ConfigurationError("You can't get `str` after indexing field. Original text was deleted.")
        return self.text
