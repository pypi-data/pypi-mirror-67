...
import mingdongtextsim
from mingdongtextsim.embeddings.embedding import Embedding
from mingdongtextsim.processors.base_processor import BaseProcessor
from mingdongtextsim.utils.get_file import get_file
from mingdongtextsim.utils.logger import logger
from mingdongtextsim.utils.tokenizer import Tokenizer
...
class WordEmbedding(Embedding):
    ...
    def _build_token2idx_from_w2v(self):
        if not self.w2v_path or not os.path.exists(self.w2v_path):
            ...
            self.w2v_path = os.path.join(mingdongtextsim.USER_DATA_DIR, untar_filename)
            if not os.path.exists(self.w2v_path):
                get_file(
                    tar_filename, url, extract=True,
                    cache_dir=mingdongtextsim.USER_DIR,
                    cache_subdir=mingdongtextsim.USER_DATA_DIR,
                    verbose=1
                )
        t0 = time.time()
