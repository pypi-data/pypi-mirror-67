...
class ServiceBootstrap(Bootstrap):
    ...
    def get_service_environment_path(self) -> str:
        ...
        It is very unlikely that you should need to change this, but if you have env stored somewhere else and this
            makes it convenient, go for it.
        :return:
        """
        return Path(Path(__file__).resolve().parents[2]).joinpath('env.json')
    ...
    def get_service_config_path(self) -> Path:
        ...
        :return:
        """
        return Path(Path(__file__).parents[2]).joinpath("config").joinpath(Environment.stage())
