        ...
        for files in BaseDir.get_dir_all_files(src):
            ...
            if source_size is not None:
                if BaseFile.get_file_size(files) >= source_size:
                    ...
                    print("process_src:%d/%d, Time total:%s, source_path: %s , dst_path:%s" % (src_file_location, src_file_num, (repr(time.time() - start_time)), files, out_path))
                    continue
            count = 0
        ...
        for files in BaseDir.get_dir_all_files(src):
            ...
            if source_size is not None:
                if BaseFile.get_file_size(files) >= source_size:
                    out_path = BaseFile.get_new_path(dst,BaseFile.get_file_name(files))
                    BaseFile.copy_file_to_file(files,out_path)
                    print("process_src:%d/%d, Time total:%.2f/%.2f, source_path: %s , dst_path:%s" % (src_file_location, src_file_num, (time.time() - start_time),(time.time() - StartTime), files, out_path))
                    continue
