        ...
        if self.upload_setting.date_conversion:
            ...
            for i in convert_columns:
                try:
                    ...
                    self.df[i] = self.df[i].fillna(dt.datetime(1990, 1, 1))
                    self.df[i] = self.df[i].dt.strftime('%Y%m%d').astype(int)
                    self.df[i] = self.df[i].replace(19900101, np.nan)
    ...
    def run(self):
        ...
        try:
            ...
            if self.upload_setting.vm_status:
                ...
                self.log('DONE')
            if self.upload_setting.clear_csv_cache:
                self.log('Removing Temporary CSV cache: {}'.format(self.save_file_path))
