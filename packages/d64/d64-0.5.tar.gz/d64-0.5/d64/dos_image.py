...
class DOSImage(object):
    ...
    def iterdir(self, include_deleted=False):
        ...
        while block:
            for offset in range(0, Block.SECTOR_SIZE, DirEntry.ENTRY_SIZE):
                entry = DirEntry(self, block.start+offset)
    ...
    def get_free_entry(self):
        ...
        while block:
            for offset in range(0, Block.SECTOR_SIZE, DirEntry.ENTRY_SIZE):
                e = DirEntry(self, block.start+offset)
                if e.is_deleted:
                    entry = e
                    break
        ...
        if entry is None:
            ...
            if block is None:
                self.bam.set_free(first_block.track, first_block.sector)
                return None

            block.data_size = Block.SECTOR_SIZE-2
            last_block.set_next_block(block)
            entry = DirEntry(self, block.start)
