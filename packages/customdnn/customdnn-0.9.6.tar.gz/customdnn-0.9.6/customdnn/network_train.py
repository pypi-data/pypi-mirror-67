...
class TrainingDeepNetwork:
    ...
    def run_nn_epochs(self, Z_all, A_all, param, cost_array, dZ_all, dW_all, Vdw, Vdb, nw_size, layer_nn, learning_rate, mini_batches, beta1, beta2, epoch_num, gradient, data_size, batch_size, dropout_percentage):
        ...
        for epoch in range(1, epoch_num+1):
            ...
            for num in range(0, num_batches):
                ...
                dZ_all, dW_all, db_all, Vdw_corrected, Vdb_corrected, Sdw_corrected, Sdb_corrected = self.backward_prop(
                    layer_nn, A_all, y_min, param, Z_all, beta1, beta2, gradient)

                param = self.param_update(dW_all, db_all, Vdw_corrected, Vdb_corrected,
    ...
    def train_network(self, x, y, learning_rate=.001, beta1=.9, beta2=.999, batch_size=32, network_size=3, gradient="Adam", epoch_num=1000, dropout_percentage=70):
        ...
        if (inp == "Y" or inp == "y"):
            ...
            mini_batches, cost_array, param, Z_all, A_all, dZ_all, dW_all, Vdw, Vdb, Sdw, Sdb = self.dnn_preprocessing(
                x, y, batch_size, layer_nn)

            data_size = x.shape[1]

            print("Network Modeling started at ", datetime.now(), "\n")
            cost_epoch_array, cost_array, auc_array, Z_all, A_all, dZ_all, dW_all, Vdw, Vdb, param = self.run_nn_epochs(
            ...
            cost_array, cost_epoch_array = self.cost_graph(
