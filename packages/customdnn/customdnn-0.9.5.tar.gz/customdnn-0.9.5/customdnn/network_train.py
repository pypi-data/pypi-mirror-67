...
class TrainingDeepNetwork: 
    ...
    def dnn_preprocessing(self,x,y,batch_size,layer_nn):
        ...
        """ It prepares neural network architecture as per the user requirements and preprocesses the data ot make it network ready. """
    ...
    def run_nn_epochs(self,Z_all, A_all,param,cost_array,dZ_all,dW_all,Vdw,Vdb,nw_size,layer_nn,learning_rate,mini_batches,beta1,beta2,epoch_num,gradient,data_size,batch_size,dropout_percentage):
        ...
        for epoch in range(1,epoch_num+1):
            ...
            incre = int(50.0 / epoch_num * i);
            sys.stdout.write('\r')
            if i != epoch_num - 1:
                    sys.stdout.write('|%s%s%s%s| %d%%' % (unitColor, '\033[7m' + ' '*incre + ' \033[27m', endColor, ' '*(49-incre), 2*incre))
            else: 
                    sys.stdout.write('|%s%s%s| %d%%' % (unitColor, '\033[7m' + ' '*20 + 'COMPLETE!' + ' '*21 + ' \033[27m', endColor, 100))    
    ...
    def cost_graph(self,cost_array,cost_epoch_array):
    ...
    def train_network(self,x,y, learning_rate=.001, beta1=.9, beta2=.999,batch_size=32,network_size=3,gradient="Adam",epoch_num=1000,dropout_percentage=70 ):
        ...
        print("You have choosen the default network having  ",network_size+1," layers with ",network_size," hidden layers.\n");
        ...
        if (inp == "Y" or inp == "y"):
            ...
            cost_array,cost_epoch_array = self.cost_graph(cost_array,cost_epoch_array);
