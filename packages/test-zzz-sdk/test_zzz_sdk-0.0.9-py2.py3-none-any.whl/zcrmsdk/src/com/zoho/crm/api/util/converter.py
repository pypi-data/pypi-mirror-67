    ...
    class Converter(ABC):
        ...
        def value_checker(self, class_name, member_name, key_details, value, unique_values_map, instance_number):
            ...
            if key_details[Constants.TYPE] != Constants.STREAM_WRAPPER_CLASS_PATH:
                ...
                if (not Constants.TYPE_VS_DATATYPE.__contains__(key_details[Constants.TYPE]) and not value.__class__.__module__ == key_details[Constants.TYPE]) or (not isinstance(value, Constants.TYPE_VS_DATATYPE.get(key_details[Constants.TYPE]))):
