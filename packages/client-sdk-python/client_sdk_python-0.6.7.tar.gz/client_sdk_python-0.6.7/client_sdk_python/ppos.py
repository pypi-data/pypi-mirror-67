...
class Ppos(Module):
    ...
    def checkDuplicateSign(self, typ, check_address, block_number, from_address=None):
        ...
        todo fill
        """
        check_address = bech32_address_bytes(check_address)
        data = rlp.encode([rlp.encode(int(3001)), rlp.encode(int(typ)), rlp.encode(check_address), rlp.encode(block_number)])
    ...
    def getDelegateReward(self, from_address, node_ids=[]):
        ...
        from_address = bech32_address_bytes(from_address)
        data = [rlp.encode(int(5100)), rlp.encode(from_address), rlp.encode(node_id_bytes)]
