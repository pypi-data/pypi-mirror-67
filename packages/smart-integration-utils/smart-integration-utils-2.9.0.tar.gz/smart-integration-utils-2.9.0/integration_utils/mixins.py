...
class BaseCredentialMixin(object):
    ...
    def get_user_info(self, request):
        ...
        elif 'HTTP_AUTHORIZATION' not in request.META and "platform_id" in request.GET:
...
class CredentialMixin(BaseCredentialMixin):
    ...
    with_settings_model = False # add for support old versions
    if hasattr(settings, 'CREDENTIAL_MODEL'):
        with_settigns_model = True
    ...
    def initial(self, request, *args, **kwargs):
        ...
        if self.with_settings_model and self.check_credential:
