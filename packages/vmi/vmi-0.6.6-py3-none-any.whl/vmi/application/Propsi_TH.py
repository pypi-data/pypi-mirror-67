...
def LeftButtonPressRelease(**kwargs):
    ...
    if kwargs['picked'] is dicom_box:
        ...
        r = vmi.askFloat(-30.0, cup_RA, 60.0, 1, 0.1)
        if r is not None:
            cup_RA = r
            cup_RA_box.draw_text('前倾角：{:.1f}°'.format(cup_RA))
        ...
        r = vmi.askFloat(10.0, cup_RI, 70.0, 1, 0.1)
        if r is not None:
            cup_RI = r
            cup_RI_box.draw_text('外展角：{:.1f}°'.format(cup_RI))
...
def NoButtonWheel(**kwargs):
    ...
    if kwargs['picked'] is bone_value_box:
        ...
        cup_RA = min(max(cup_RA + 0.1 * kwargs['delta'], -30), 60)
        cup_RA_box.draw_text('前倾角：{:.1f}°'.format(cup_RA))
        update_cup_orientation()
        update_cup()
        clear_guide_plate()
    elif kwargs['picked'] is cup_RI_box:
        cup_RI = min(max(cup_RI + 0.1 * kwargs['delta'], 10), 70)
        cup_RI_box.draw_text('外展角：{:.1f}°'.format(cup_RI))
        update_cup_orientation()
        update_cup()
        clear_guide_plate()
