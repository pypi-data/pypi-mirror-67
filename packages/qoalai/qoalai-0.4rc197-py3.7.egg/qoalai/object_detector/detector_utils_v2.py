    ...
    def yolo_loss(self, outputs: [], labels: []):
        ...
        for i in range(3):
            ...
            for idx, val in enumerate(self.anchor[border_a:border_b]):
                ...
                sz_loss =  self.size_loss_alpha * (mse_loss_sum(a, b) + mse_loss_sum(c, d))
                ...
                if self.is_multilabel: 
                    ...
                    for a, b in zip(additional_class_pred, additional_class_label):
                        additional_class_loss.append(self.class_loss_alpha * mse_loss_sum(tf.nn.softmax(a), b))
                ...
                if self.is_multilabel: 
                    for index, (a, b) in enumerate(zip(additional_class_pred, additional_class_label)):
                        additional_class_acc_total[index] += calculate_acc(tf.nn.softmax(a), b)
