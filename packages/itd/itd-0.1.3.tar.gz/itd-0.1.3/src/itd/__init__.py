...
# Had some report of readline.rl missing.
# IDK how readline works w/o pyreadline
# so I just ensure pyreadline is used:
import pyreadline
import readline
...
class Engine(ITD_Item):
    ...
    def consume_power(self):
        # clean up
        trunk = self._ITD_game.Trunk
        for pc in self._power_cells:
            if pc in trunk.content:
                trunk.content.remove(pc)
        self._power_cells[:] = [
            pc for pc in self._power_cells
            if isinstance(pc, PowerCell) and pc.energy
        ]

        game = self._ITD_game

        if self._power_cells:
            pc = self._power_cells[-1]
            pc.energy -= 1
            ...
            if not pc.energy:
                ...
                self._power_cells.remove(pc)

        if not self._power_cells:
            game.show_message(
                "---",
                "DAMN ! No more power !",
    ...
    def hud(self):
        if self._is_on and self._power_cells:
            self.consume_power()
        return '{} [{}] [Power:{}]'.format(
            self.__class__.__name__,
...
class TurnEngineOn(ITD_Stage):
    ...
    def react(self, *args, **kwargs):
        if self.game.Engine.power_cells:
            ...
            self.game.show_message(
                "---",
                "The engine has enough batteries.",
                "Now turn it on !",
                "---",
            )
