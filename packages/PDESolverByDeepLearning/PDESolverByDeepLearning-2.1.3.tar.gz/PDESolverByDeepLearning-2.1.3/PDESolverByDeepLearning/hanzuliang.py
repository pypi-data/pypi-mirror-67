...
def PDESolver(domain, n, realSolution, StructureOfNeuralNetwork, ImplicitSchemeOfEquation,
    ...
    BClossList = []                                                 #储存边界损失
    for i in range(len(DirichletBCValue)):
        xi = DirichletBCPoint[i]
        ui = DirichletBCValue[i]
        print(x_train)
    ...
    if realSolution != None:                                        #如果知道真解则画预测解曲线、真解曲线和误差图
    ...
    else:                                                           #如果真解未知,则只画预测解曲线,不画真解曲线和误差图
        fig = plt.figure("Prediction curve")
        plt.plot(x_train, y_output, 'k.')

    #不管知不知道真解,Loss都要画
    fig3 = plt.figure("Loss")                                       #可视化损失函数
    BClossList = []                                                 #储存边界损失
    for i in range(len(DirichletBCPoint)):
        xi = DirichletBCPoint[i]
        ui = DirichletBCValue[i]
        BClossList.append((y_output[x_train.tolist().index(xi)]-ui) ** 2)
    BCloss = sum(BClossList)
    plt.plot(x_train, f_output + BCloss)
    plt.show()
