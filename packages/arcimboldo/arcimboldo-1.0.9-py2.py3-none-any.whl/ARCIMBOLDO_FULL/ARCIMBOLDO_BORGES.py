...
def startARCIMBOLDO_BORGES(BorData, isShredder, input_bor, DicParameters={}, DicGridConn={}, cm=None, sym=None,
    ...
    try:
        Config_1 = SELSLIB2.GenerateConfigParseObject(input_bor, is_file=False)
    ...
    try:
        ...
        seq = Config.get("GENERAL", "seq_path")
        ...
        if seq != None and seq.endswith(".seq"):
            seq = os.path.abspath(seq)
        else:
            seq = ""
        if seq and not os.path.exists(seq):
            toExit=True
            print('The path to a sequence file given does not exist, please check')
    ...
    if doTest:
        ...
        fneed, tNCS_bool = SELSLIB2.anisotropyCorrection_and_test(cm=cm, sym=sym, DicGridConn=DicGridConn,
                                                                              ...
                                                                              hkl=hkl, ent=ent, seq=seq, formfactors=formfactors,
...
#updateAnomParamDic(hkl=None, mtz=None, current_directory=None, cell_dim=None, sg_number=1,otherAnomParamDic=None, st     artExpAnomDic=None)
    ...
    # SHELXE LINES
    linsh, linsh_last, _ = SELSLIB2.get_shelxe_line(Config, job_type, resolution, seq, coiled_coil, fneed)
        ...
        for clusi in range(len(orderedClusters)):
            ...
            if topExp_run is not None:
                ...
                if alixe:
                    ...
                    #print('SHERLOCK input_mode',input_mode)
                    values = SELSLIB2.readCCValFromSUM(fileToCheck)
                    #print('SHERLOCK values[0]',values[0])
                    #print('SHERLOCK len(values[0])', len(values[0]))
                    if len(values[0]) == 1:
                    ...
                    if not skip_cluster_alixe:
                        ...
                        if alixe_mode == 'monomer':
                            # input_mode, wd, path_rotclu, type_run
