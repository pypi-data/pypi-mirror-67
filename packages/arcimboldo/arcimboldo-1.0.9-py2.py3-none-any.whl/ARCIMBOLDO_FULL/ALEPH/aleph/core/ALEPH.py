...
def parse_hhr_file (pdb_model, g, write_pdb, hhr_file, path_base='./'):
    ...
    hhpred_nomenclature_dict = {'|': 20, '+': 30, '.': 40, '-': 50, '=': 60, ' ': 60}
        ...
        for i, chain in enumerate(Bio.PDB.Selection.unfold_entities(strucc[model.get_id()], "C")):
            ...
            for i, frag in enumerate(g.vs):
                ...
                frag_alignment = [alignment[resi[3][1] + conversion] for resi in frag['reslist'] if (
                        (resi[3][1] + conversion) >= 0 and (resi[3][1] + conversion) < len(alignment))]
                frag_alignment = [hhpred_nomenclature_dict[symbol] for symbol in frag_alignment]
                frag['alignment'] = frag_alignment

                if frag['alignment'] == []:
                    frag['alignment'] = [hhpred_nomenclature_dict[' '] for y in frag['reslist_aligned']]
            ...
            for frag in g.vs:
                ...
                for e, residue in enumerate(frag['reslist_aligned']):
                    ...
                    if set1.issubset(set2):
                        try:
                            atm.set_bfactor(frag['alignment'][e])
                            reference_1.append(atm)
                            encontrado = True
                        except:
                            atm.set_bfactor(hhpred_nomenclature_dict[' '])
                            reference_1.append(atm)
            if not encontrado:
                atm.set_bfactor(60)
                reference_1.append(atm)

        pdbmod_1, cnv_1 = Bioinformatics.get_pdb_from_list_of_atoms(reference_1, renumber=False,
                                                                    uniqueChain=False)
