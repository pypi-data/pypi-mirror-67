...
async def post(parent, ctx: commands.Context, **kwargs):
    """
    Helper to determine whether or not an embed should be used
    """

    await WidgitEmbed(parent).post(ctx, **kwargs)
...
class WidgitEmbed:
    ...
    async def post(self, ctx: commands.Context, **kwargs):
        """
        Helper to determine whether or not an embed should be used
        """
        ...
        raise WidgitEmbedException(
            self.parent._("I doesn't have permission to post embeds and no fallback post was given!")
        )
