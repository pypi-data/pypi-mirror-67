...
class _TorchTrainer(object):
    ...
    def _loop(self, mode):
        ...
        for batch_index, batch_samples in enumerate(dataset_loader):
            ...
            # computing loss
            batch_loss = self.experiment.loss(y_pred, y_true.long())
    ...
    def test(self):
        ...
        if self.experiment.n_batches_valid > 0:
            self.experiment._model = torch.load("Net_best_loss.pt")
        self._loop(mode=2)
        if self.experiment.verbose >= 1:
            expression = f"loss: {self.test_results['loss']}\n"
            if self.experiment.classification_flag:
                expression += f"accuracy: {self.test_results['acc']}\n"
                expression += f"conf matrix:\n{self.test_results['cm']}"
            else:
                expression += f"r2 score: {self.test_results['r2']}"
            self.experiment.logger.print(
                expression,
                up_extra_lines=1,
                timestamp_flag=False
