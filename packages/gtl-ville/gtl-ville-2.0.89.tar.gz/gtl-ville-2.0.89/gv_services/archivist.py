...
class Archivist:
    ...
    async def store_data(self, pbtrafficdata, datatype, pbtimestamp):
        ...
        try:
            ...
            coros = [filestorage.write_data(self.basedatapath, pbtrafficdata.trafficdata, datatype, timestamp), ]
    ...
    async def _store_data_quality(self, pbtrafficdata, datatype):
        success = False
        try:
            data = (await protobuf.decode_traffic_data(pbtrafficdata))[SAMPLES]
            dataquality = None
            freq = 1
            if datatype == METROPME:
                dataquality = self._compute_metropme_quality(data)
            elif datatype == TOMTOMFCD or datatype == ROADS:
                dataquality = self._compute_tomtomfcd_quality(data)
    ...
    def _compute_tomtomfcd_quality(data):
        confidences = [sample.get(CONFIDENCE, 0) for sample in data.values()]
