...
class CommonActions():
    ...
    def get_user_agents(self, type_, number):
        if type_ == 'mobile':
            mobile_user_agents = self.file_to_list(pkg_resources.resource_filename(__name__, 'data/mobile_user_agents.txt'))
            return random.sample(mobile_user_agents, number)

        elif type_ == 'desktop':
            desktop_user_agents = self.file_to_list(pkg_resources.resource_filename(__name__, 'data/desktop_user_agents.txt'))
