...
async def _dmi_decode(hub, *args: Tuple[str]):
    binary = (
        shutil.which("dmidecode")
        or shutil.which("smbios")
        or os.path.exists("/usr/sbin/smbios")
    )
    ret = await hub.exec.cmd.run([binary] + [x for x in args])
    return ret.stdout.strip()


async def get(hub, dmi_string: str, clean: bool = True) -> str:
    """
    ...
    """
    val = await _dmi_decode(hub, "-s", dmi_string)

    # Cleanup possible comments in strings.
    val = "\n".join([v for v in val.split("\n") if not v.startswith("#")])
    if val.startswith("/dev/mem") or (
        clean and not await _dmi_isclean(dmi_string, val)
    ):
        return ""
