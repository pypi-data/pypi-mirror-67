...
class NabuValidator(object):
    ...
    def check_slice_indexes(self):
        ...
        what = (
            ("start_x", "end_x", nx),
            ("start_y", "end_y", nx), # TODO support non-square slices
            ("start_z", "end_z", nz)
        )
    ...
    def remove_unused_radios(self):
        ...
        if self.dataset_infos.dataset_scanner.type != "edf":
            return
        # Extraneous projections are assumed to be on the end
        used_radios_range = range(self._projs_indices[0], self.dataset_infos.n_angles)
        radios_not_used = []
        for idx in self.dataset_infos.projections.keys():
            if idx not in used_radios_range:
                radios_not_used.append(idx)
        for idx in radios_not_used:
            self.dataset_infos.projections.pop(idx)
    ...
    def handle_binning(self):
        ...
        self.projections_subsampling = dataset_cfg["projections_subsampling"]
        if self.projections_subsampling > 1:
            self.dataset_infos.projections = subsample_dict(
                self.dataset_infos.projections, self.projections_subsampling
            )
            self._projs_indices = sorted(self.dataset_infos.projections)
            self.dataset_infos.reconstruction_angles = self.dataset_infos.reconstruction_angles[::self.projections_subsampling]
            self.dataset_infos.n_angles = len(self.dataset_infos.projections)
            self.dataset_infos.flats = _subsample_frames_indices(
                self.dataset_infos.flats, self.projections_subsampling
            )
            self.dataset_infos.darks = _subsample_frames_indices(
                self.dataset_infos.darks, self.projections_subsampling
            )
        if self.binning != (1, 1):
...
def _subsample_frames_indices(frames, subsampling_factor):
    new_frames = {}
    for frame_idx, frame_url in frames.items():
        new_frames[frame_idx // subsampling_factor] = frame_url
    return new_frames
