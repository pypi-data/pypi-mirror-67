...
class SinoProcessing(object):
    ...
    def __init__(self, sinos_shape=None, radios_shape=None, rot_center=None, halftomo=False, vertical_shifts=None):
        """
        ...
        vertical_shifts: array
            Define vertical sample/detector shifts
        """
        self._get_shapes(sinos_shape, radios_shape)
        self._set_rot_center(rot_center)
    ...
    def _set_rot_center(self, rot_center):
    ...
    def _set_halftomo(self, halftomo):
        ...
        if not(halftomo):
            return
        assert (self.n_angles % 2) == 0, "Half tomography: expected an even number of angles (got %d)" % n_angles
    ...
    def _radios_to_sinos_halftomo(self, radios, sinos):
        ...
        n_a2 = n_a // 2
        rot_center_int = int(round(self.rot_center))
        out_dwidth = 2 * rot_center_int
        if sinos is not None:
            ...
            sinos = np.zeros((n_z, n_a2, out_dwidth), dtype=np.float32)
        for i in range(n_z):
            sinos[i][:] = convert_halftomo(sinos[i], rot_center_int)
        return sinos
    ...
    def radios_to_sinos(self, radios, output=None, copy=False):
        ...
        if self.halftomo:
            self._radios_to_sinos_halftomo(radios, output)
