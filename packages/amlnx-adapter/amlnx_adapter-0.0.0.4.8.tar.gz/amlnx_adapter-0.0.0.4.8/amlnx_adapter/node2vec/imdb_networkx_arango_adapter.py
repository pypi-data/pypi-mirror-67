...
class ArangoDB_Networkx_Adapter(Networkx_Arango_Adapter):
    ...
    def is_valid_config(self, graph_config):
        ...
        if not gp.DB_DATA_QUERY in graph_config:
            ...
            valid_config = False
        if not gp.DB_VERTICES in graph_config:
            print("vertex names in schema are not provided")
            valid_config = False
        if not gp.DB_EDGE_FROM in graph_config:
            print("edge from attribute in schema is not provided")
            valid_config = False
        if not gp.DB_EDGE_TO in graph_config:
            print("edge to attribute in schema is not provided")
            valid_config = False
        return valid_config
    ...
    def create_networkx_graph(self):
        ...
        query = self.cfg['queries']['load_data']
                #get vertex names
        vertices = self.cfg[gd.DB_SCHEMA][gd.DB_VERTICES]
        #get from vertices for the edges
        fvl = self.cfg[gd.DB_SCHEMA][gd.DB_EDGE_FROM]
        # get the edge collection to create
        edge_col_name = self.cfg[gd.DB_SCHEMA][gd.DB_EDGE_COL_NAME]
        #get to vertices for the edges
        tvl = self.cfg[gd.DB_SCHEMA][gd.DB_EDGE_TO]
