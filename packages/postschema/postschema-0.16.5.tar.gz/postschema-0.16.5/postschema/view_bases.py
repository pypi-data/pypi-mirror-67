...
from .fields import (
    ...
    AutoSessionOwner, AutoSessionField,
    AutoSessionForeignResource, RangeDTField
)
...
class ViewsClassBase(web.View):
    ...
    def post_init(cls, joins):
        ...
        autoowner_fields = {
            field: fieldval.session_key for field, fieldval in declared_fields
            if isinstance(fieldval, AutoSessionOwner)
        }
        ...
        cls.patch_schema = cls.put_schema = write_schema(use='write',
                                                         partial=True,
                                                         exclude=update_excluded,
                                                         autosession_fields=autoowner_fields)
        ...
        cls.schema_variants = {
            ...
            'authed': {
                'get_schema': read_schema(
                    use='read', joins=auth_get_joins, only=auth_get_by, partial=True,
                    autosession_fields=autoowner_fields),
                'list_schema': read_schema(
                    use='read', joins=auth_list_joins, only=auth_list_by, partial=True,
                    autosession_fields=autoowner_fields),
                'delete_schema': read_schema(
