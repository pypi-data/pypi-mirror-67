...
def oauth_access_token(server, grant_type, client_id, client_secret, username, password):
    ...
    try:
        ...
        url = f"/oauth/token?grant_type={quote(grant_type)}&scope=all&username={quote(username)}&password={quote(password)}"
        authorization = get_authorization_header(client_id, client_secret)
        r = do_service(url, app_name=server, method="POST", auth=None, headers={"Authorization": authorization})
