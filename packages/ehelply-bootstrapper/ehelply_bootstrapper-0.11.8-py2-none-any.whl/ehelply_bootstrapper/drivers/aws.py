...
class AWS(Driver):
    ...
    def __init__(self, service_gatekeeper_key: str, connection_details: ConnectionDetails = None, verbosity: int = 0):
        ...
        self.aws_access_key_id = enc.decrypt(State.config.aws.auth.access_key_id.encode(STRING_ENCODING))
        self.aws_secret_access_key = enc.decrypt(State.config.aws.auth.access_key.encode(STRING_ENCODING))
