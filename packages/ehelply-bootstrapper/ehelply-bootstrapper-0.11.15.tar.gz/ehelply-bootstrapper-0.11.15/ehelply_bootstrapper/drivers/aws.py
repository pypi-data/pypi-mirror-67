...
from ehelply_bootstrapper.utils.cryptography import Encryption, STRING_ENCODING
...
class AWS(Driver):
    ...
    def __init__(self, service_gatekeeper_key: str, connection_details: ConnectionDetails = None, verbosity: int = 0):
        ...
        enc: Encryption = Encryption([service_gatekeeper_key.encode(STRING_ENCODING)])
        self.aws_access_key_id = enc.decrypt_str(State.config.aws.auth.access_key_id.encode(STRING_ENCODING))
        self.aws_secret_access_key = enc.decrypt_str(State.config.aws.auth.access_key.encode(STRING_ENCODING))
