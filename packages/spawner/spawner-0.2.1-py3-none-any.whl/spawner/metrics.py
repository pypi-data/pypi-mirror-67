...
def max_drawdown(token, ticker, purchase_date, sale_date, purchase_price): 
    url = "https://spawnerapi.com/max_drawdown/" + ticker + "/" + purchase_date + "/" + sale_date + "/" + purchase_price + "/" + token
    response = requests.get(url).text
    return round(float(response),2)

def sharpe(token, ticker, purchase_date, sale_date, purchase_price): 
    url = "https://spawnerapi.com/sharpe/" + ticker + "/" + purchase_date + "/" + sale_date + "/" + purchase_price + "/" + token
    response = requests.get(url).text
    return round(float(response),2)
