...
def calibration_check(root):
    calibration_files = glob.glob(root + '\\Parameter\\Baseline\\*')
    calibration_dict = defaultdict()

    for file in calibration_files:
        type = os.path.split(file)[-1].split()[0]
        value = os.path.splitext(os.path.split(file)[-1].split()[-1])[0]
        if calibration_dict.get(type):
            calibration_dict[type][value] = pd.read_csv(file)
        else:
            calibration_dict[type] = {value: pd.read_csv(file)}
...
def helper(**kwargs):
    ...
    for file in data_files:
        ...
        name = naming(file, isotherm_pattern, topic_pattern)

        if method == 'ms':
            ion = os.path.splitext(file.split('\\')[-1])[0]
            name = ' '.join([name, '-', ion])

        frame_dictionary[name] = data_frame
...
def naming(file, isotherm_pattern, topic_pattern):
    name = re.search(topic_pattern, file)[1]
    name = name.replace('Topic', '').strip()
    if 'Isotherm' in file:
        isotherm = re.search(isotherm_pattern, file)[2]
        name = ' '.join([name, '-', isotherm])
    else:
        name = ' '.join([name, '-', 'Gradient'])
    return name
