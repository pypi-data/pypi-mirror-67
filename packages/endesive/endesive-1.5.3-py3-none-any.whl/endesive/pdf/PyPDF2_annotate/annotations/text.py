...
class FreeText(Annotation):
    ...
    def make_font_file_object(tt_font):
        ...
        return IndirectPdfDict(stream=data, Filter=PdfName("FlateDecode"))
    ...
    def make_cid_to_gid_map_object(tt_font):
        ...
        return IndirectPdfDict(stream=cid_to_gid_map, Filter=PdfName("FlateDecode"))
    ...
    def add_additional_resources(self, resources):
        font_dict = IndirectPdfDict()
        if 0:
            font_dict[PdfName(PDF_ANNOTATOR_FONT)] = self.make_font_object()
        else:
            font_dict[PdfName(PDF_ANNOTATOR_FONT)] = self.make_composite_font_object(
                HELVETICA_PATH
            )
