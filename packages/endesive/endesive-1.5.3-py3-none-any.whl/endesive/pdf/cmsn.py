    ...
    def _extend(self, obj):
        ...
        for k, v in obj.items():
            if isinstance(v, pdf.DictionaryObject):
                ...
                for i in v:
                    if isinstance(i, pdf.DictionaryObject):
                        if i.indirect:
                            i = self._extend(i)
                            i = self._addObject(i)
                        else:
    ...
    def makepdf(self, prev, udct, algomd, zeros):
        ...
        if box is not None:
            ...
            if annotationtext is not None:
                a_n18 = po.DictionaryObject()
                a_n18ref = self._addObject(a_n18)
                a_n18.update(
                    {
                        po.NameObject("/Type"): po.NameObject("/Font"),
                        po.NameObject("/Subtype"): po.NameObject("/Type1"),
                        po.NameObject("/BaseFont"): po.NameObject("/Helvetica"),
                        po.NameObject("/Encoding"): po.NameObject("/WinAnsiEncoding"),
                    }
                )
                a_n19 = po.DictionaryObject()
                a_n19ref = self._addObject(a_n19)
                a_n19.update({po.NameObject("/PDFANNOTATORFONT1"): a_n18ref})
                a_n20 = po.DictionaryObject()
                a_n20ref = self._addObject(a_n20)
                a_n20.update(
                    {
                        po.NameObject("/ProcSet"): po.NameObject("/PDF"),
                        po.NameObject("/Font"): a_n19ref,
                    }
                )
                a_n21 = po.DecodedStreamObject()
                a_n21ref = self._addObject(a_n21)
                a_n21.update(
                    {
                        po.NameObject("/BBox"): po.ArrayObject(
                            [
                                po.NumberObject(box[0]),
                                po.NumberObject(box[1]),
                                po.NumberObject(box[2]),
                                po.NumberObject(box[3]),
                            ]
                        ),
                        po.NameObject("/Resources"): a_n20ref,
                        po.NameObject("/Matrix"): po.ArrayObject(
                            [
                                po.NumberObject(1),
                                po.NumberObject(0),
                                po.NumberObject(0),
                        ...
                        po.NameObject("/FormType"): po.NumberObject(1),
                    }
                )
                s = "ąćęłńóśżź".encode("UTF-16BE")
                data = [
                    "2 J".encode(),
                    "0.57 w".encode(),
                    "BT /PDFANNOTATORFONT1 12.00 Tf ET".encode(),
                    "BT 461 62 Td (Dokument) Tj ET".encode(),
                    "BT 471 47.6 Td (podpisany cyfrowo) Tj ET".encode(),
                    "BT 471 33.2 Td (".encode() + s + ") Tj ET".encode(),
                ]
                a_n21.setData(b"\n".join(data))
                a_n = po.DictionaryObject()
                a_n.update({po.NameObject("/N"): a_n21ref})
                a_bs = po.DictionaryObject()
                a_bs.update(
                    {
                        po.NameObject("/Type"): po.NameObject("/Border"),
                        po.NameObject("/W"): po.NumberObject(0),
                        po.NameObject("/S"): po.NameObject("/S"),
                    }
                )
                obj13.update(
                    {
                        po.NameObject("/BS"): a_bs,
                        po.NameObject("/C"): po.ArrayObject(),
                        po.NameObject("/Contents"): po.createStringObject(
                            annotationtext
                        ),
                        po.NameObject("/DA"): po.NameObject(
                            "(0 0 0 rg /PDFANNOTATORFONT1 12 Tf)"
                        ),
                        po.NameObject("/Rect"): po.ArrayObject(
                            [
                                po.NumberObject(box[0]),
