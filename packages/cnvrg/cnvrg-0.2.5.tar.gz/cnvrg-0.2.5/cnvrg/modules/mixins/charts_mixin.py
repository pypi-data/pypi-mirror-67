...
class ChartsMixin(object):
    ...
    def log_chart(self, key: AnyStr, data, title: AnyStr=None, x_axis: AnyStr=None, y_axis: AnyStr=None, y_ticks: List=None, x_ticks: List=None):
        ...
        if data[0].chart_type() == 'heatmap':
            heat_map = data[0]
            if heat_map.color_stops:
                additional_params["stops"] = heat_map.color_stops
            if heat_map.max:
                additional_params["max"] = heat_map.max
            if heat_map.min:
                additional_params["min"] = heat_map.min
    ...
    def __create_chart(self, key, chart_type, serieses, title, x_axis, y_axis, x_ticks, y_ticks, **kwargs):
        data = {
            "chart": {
