...
def checkIfGoodEnough(
    ...
    maxSimilarity = getMaxSimilarity()
            ...
            if subsWord == scriptWord and len(subsValue["content"]) == len(
                ...
                for scriptIndex, scriptContent in enumerate(scriptContentNoDuplicate):
                    ...
                    for subsIndex, subsContent in enumerate(subsValue["content"]):
                        ...
                        goodEnough, maxSimilarity = checkIfGoodEnough(
                            nlp,
                            scriptContent,
                            subsContent,
                            subsWord,
...
"""
    if it has highest similarity BUT length of script and subtitle 
    is the same AND the script with same order as subtitle has a high
    enough similarity (albeit not higher than current) AND similar enough
    percentage, still just keep that similarity
