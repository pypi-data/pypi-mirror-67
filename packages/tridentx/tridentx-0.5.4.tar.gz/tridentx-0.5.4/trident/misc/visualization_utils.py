...
def plot_bbox(x, img, color=None, label=None, line_thickness=None):
    ...
    draw.rectangle(((x[0], x[1]), (x[2], x[3])),outline=color,width=2)
    draw.rectangle(((x[0], x[1]-18), (x[0]+45,  x[1])), fill=color, width=2)
    fontcolor=(255,255,255)
    avg_color=np.mean(np.array(list(color)))
    if avg_color>150:
        fontcolor=(0,0,0)

    draw.text((x[0]+6,  x[1]- 15), u'{0}'.format(label),fill=fontcolor, font=ImageFont.truetype(fonts[names.index( 'Microsoft YaHei')],10))
