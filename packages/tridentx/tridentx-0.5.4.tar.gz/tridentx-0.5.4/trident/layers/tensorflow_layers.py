...
from ..backend.tensorflow_backend import Layer,  Sequential
...
class Upsampling2d(Layer):
    ...
    def forward(self, *x):
        x = enforce_singleton(x)
        new_shape=x.shape.as_list()[1:3]
        if self.scale_factor is not None and isinstance(self.scale_factor,(int,float)):
            new_shape[1]=new_shape[1]*self.scale_factor
            new_shape[2] = new_shape[2] * self.scale_factor
