...
class _BarChartRace:
    ...
    def __init__(self, df, filename, orientation, sort, label_bars, use_index, 
                 steps_per_period, period_length, title, figsize, dpi, fig, kwargs):
        self.df = df
    ...
    def validate_params(self):
        ...
        if self.fig is not None and not isinstance(fig, plt.Figure):
...
def bar_chart_race(df, filename, orientation='h', sort='desc', label_bars=True, 
                   use_index=True, steps_per_period=10, period_length=500, title=None, 
                   figsize=(7, 3.5), dpi=120, fig=None, **kwargs):
    '''
    ...
    **kwargs : key, value pairs
    ...
    bcr = _BarChartRace(df, filename, orientation, sort, label_bars, use_index, steps_per_period, 
                        period_length, title, figsize, dpi, fig, kwargs)
    bcr.make_animation()
