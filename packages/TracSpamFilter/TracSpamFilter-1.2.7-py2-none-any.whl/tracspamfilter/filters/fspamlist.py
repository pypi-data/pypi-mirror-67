...
class FSpamListFilterStrategy(Component):
    ...
    def test(self, req, author, content, ip):
        ...
        try:
            ...
            self.log.warn("Error parsing response from FSpamList: %s\n"
                          "Response:\n%s", e, resp)
            return
        ...
        if len(reason):
            return (-abs(self.karma_points) * len(reason),
                    N_("FSpamList says this is spam (%s)"),
                    ("; ".join(reason)))
    ...
    def _send(self, req, author, ip):
        ...
        url = 'http://www.fspamlist.com/api.php?spammer=' + request + \
              '&key=' + self.api_key
        urlreq = urllib2.Request(url, None, {'User-Agent': user_agent})
