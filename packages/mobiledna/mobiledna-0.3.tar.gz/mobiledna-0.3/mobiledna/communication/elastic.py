...
def split_pipeline(ids: list, dir: str,
    ...
    for index, id in enumerate(ids):
        log(f"Getting started on ID {id} ({index}/{len(ids)})", title=True)
...
if __name__ in ['__main__', 'builtins']:
    ...
    time_range = ('2020-03-01T00:00:00.000', '2020-04-01T00:00:00.000')

    ids = ids_from_server(index='appevents', time_range=time_range)
    split_pipeline(ids=list(ids.keys()), subfolder=True,
                   dir=os.path.join(hlp.DATA_DIR,'corona_new'),
                   time_range=time_range,
                   indices=('appevents',),
                   parquet=True,
                   csv_file=False)
