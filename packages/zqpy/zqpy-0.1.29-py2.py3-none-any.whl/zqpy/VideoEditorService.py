...
class VideoEditorServiceClass():
    ...
    def video_merge_all_video_by_list(self, video_list, save_path, suffix_name='.mp4'):
        ...
        try:
            # 拼接视频
            final_clip = self.concatenate_videoclips(target_list)
            # 生成目标视频文件
    ...
    def video_resize(self, video_path, save_path, width, height):
        ...
        final_clip.to_videofile(save_path, fps=25, remove_temp=True)
        return result_param(True, save_path, final_clip, None, '')
    ...
    def video_add_log(self, videoPath, save_path, logoPngPath = None, logoTextStr = None, Opacity = 1):
        ...
        if logoTextStr != None:
            FONT_URL = r'C:\Windows\Fonts\simhei.ttf'    #C:\Windows\Fonts\STXINGKA.TTF'
