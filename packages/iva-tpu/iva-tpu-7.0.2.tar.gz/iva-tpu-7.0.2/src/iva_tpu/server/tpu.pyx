...
cdef class TPUDevice:
    ...
    def set_features(self, features: List[np.ndarray] or Dict[str, np.ndarray]):
        ...
        if type(features) == dict:
            ...
            for name, tensor in features.items():
                ...
                feature_tensors[index] = c_tpu.tpu_program_make_input_tpu_tensor(self.c_tpu_program, index)
    ...
    def get_results(self, dtype = np.int8) -> List[np.ndarray]:
        ...
        try:
            ...
            for i in range(output_count):
                ...
                arr = np.frombuffer(c_buffer[:result_tensors[i].size], dtype=np.dtype(dtype)).reshape(shape)
                if self.io_dict_mode == 1:
    ...
    def run_inference(self):
        with nogil:
            ...
            c_tpu.tpu_inference_wait(self.c_tpu_device)
