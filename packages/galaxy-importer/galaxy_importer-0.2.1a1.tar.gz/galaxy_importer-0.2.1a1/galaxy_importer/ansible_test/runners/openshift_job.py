...
class Job(object):
    ...
    def cleanup(self):
        ...
        self.log.info(f'Deleted job {self.name}')

        for pod in self.get_pods():
            pod_name = self.get_pod_name(pod)
            requests.delete(
                f'{self.pods_url}/{pod_name}', headers=self.auth_header, verify=self.ca_path)
            self.log.info(f'Deleted pod {pod_name}')

            status = pod['status']['phase']
            self.log.info(f'status_phase={status}')
            if status != 'Succeeded':
                self.log.warning(f'status_phase={status}')
                self.log.warning(pod['status']['containerStatuses'])
            if status == 'Failed':
                # TODO: consider maybe just logging here not raise?
