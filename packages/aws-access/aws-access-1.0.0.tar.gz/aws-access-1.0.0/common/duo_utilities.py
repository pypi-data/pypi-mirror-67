...
def get_payload(form_response_soup, user_name, password):
    ...
    for inputtag in form_response_soup.find_all(re.compile('(INPUT|input)')):
        ...
        if "pf.username" in name.lower():
            ...
            # Populate the parameter with the existing value (gets the hidden fields in the login form)
            payload[name] = value
    return payload

def get_action_url(form_response_soup):
    '''
    :param form_response_soup: The response after calling the action url from the initial response
...
def get_duo_attributes(form_response_soup):
    ...
    '''
    input_tag = form_response_soup.find_all('script', {"type": "application/javascript"})
    duo_init = ast.literal_eval(input_tag[1].get_text().split('(', 1)[1].split(')')[0])
...
def get_assertion(form_response_soup):
    ...
    :return: sml assertion, this is what is used to connect to aws to get the temp token
    '''
    assertion = ''
