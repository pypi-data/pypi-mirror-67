...
class Minlsh(object):
    ...
    def _computesignaturematrix(self, rows, shape, siglen):
        ...
        hashvalues = np.array([[hashfunc[i](r) for r in xrange(ncols)] for i in xrange(siglen)])
    ...
    def predict(self, threshold, minclustersize=2):
        ...
        for r in xrange(0, sig.shape[0], bandsize):
            ...
            similars.update(set(tuple(v) for v in itervalues(buckets) if len(v) >= minclustersize))
        return similars
