...
class NormalizerFunctionTestCase(unittest.TestCase):
    def test_unormalize(self):
        self.assertEqual(lunormalize(u"bépoèàÀêùï"), u"bepoeaaeui")

    def test_simplify(self):
        self.assertEqual(
            simplify(u"J'aime les frites, les pommes et les" u" scoubidous !", FRENCH_LEMMAS),
            u"aimer frites pomme scoubidou",
        )

    def test_tokenize(self):
        self.assertEqual(
            tokenize(u"J'aime les frites !"), [u"J'", u"aime", u"les", u"frites", u"!",]
        )

    def test_lemmatizer(self):
        self.assertEqual(lemmatized(u"sacré rubert", FRENCH_LEMMAS), u"sacré rubert")
        self.assertEqual(lemmatized(u"J'aime les frites !", FRENCH_LEMMAS), u"je aimer le frite")
        self.assertEqual(lemmatized(u", J'aime les frites", FRENCH_LEMMAS), u"je aimer le frite")
    ...
    def test_format(self):
        ...
        self.assertEqual(
            rgxformat(string, regex, output), u"Hugo, Victor (26 fev 1802 - 22 mai 1885)"
        )

        string = u"http://perdu.com/42/supertop/cool"
        regex = r"http://perdu.com/(?P<id>\d+).*"
        output = u"%(id)s"
        self.assertEqual(rgxformat(string, regex, output), u"42")
...
class NormalizerObjectTestCase(unittest.TestCase):
    ...
    def test_simplify_record(self):
        ...
        self.assertEqual(
            ["a1", u"aimer frites pomme scoubidou"],
            normalizer.normalize(["a1", u"J'aime les frites, les pommes et les scoubidous !"]),
        )

    def test_tokenize(self):
        normalizer = TokenizerNormalizer()
        self.assertEqual(
            [u"J'", u"aime", u"les", u"frites", u"!",], normalizer.normalize(u"J'aime les frites !")
        )

    def test_tokenize_record(self):
        normalizer = TokenizerNormalizer(attr_index=1)
        self.assertEqual(
            ["a1", [u"J'", u"aime", u"les", u"frites", u"!",]],
    ...
    def test_lemmatizer_record(self):
        ...
        self.assertEqual(["a1", u"sacré rubert"], normalizer.normalize(["a1", u"sacré rubert"]))
        self.assertEqual(
    ...
    def test_regexp_record(self):
        ...
        self.assertEqual(
            ["a1", u"42"], normalizer.normalize(["a1", u"http://perdu.com/42/supertop/cool"])
        )
