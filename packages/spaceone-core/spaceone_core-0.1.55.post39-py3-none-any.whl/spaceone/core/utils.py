...
def parse_timedelta_query(query):
    try:
        time_info, include_operator = _parse_timedelta_from_regex(query)
        ...
        raise ValueError(f'Timedelta query is invalid. ({query})')


def _convert_time_delta(time_delta_number, time_delta_unit):
    _time_delta_map = {
        's': 'seconds',
        'm': 'minutes',
        'h': 'hours',
...
def _convert_base_time(time_str):
...
def _parse_timedelta_from_regex(query):
