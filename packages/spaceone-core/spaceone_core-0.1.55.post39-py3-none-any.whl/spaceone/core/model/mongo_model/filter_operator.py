...
def _timedelta_resolver(key, value, operator, is_multiple, is_exact_field):
    try:
        dt = utils.parse_timedelta_query(value)
    except Exception as e:
        raise ERROR_DB_QUERY(reason=f'The value of timedelta_{operator} operator is invalid. (value = {value})')
    return Q(**{f'{key}__{operator}': dt})
...
FILTER_OPERATORS = {
    ...
    'timediff_gt': (_timedelta_resolver, 'gt', False),
    'timediff_lt': (_timedelta_resolver, 'lt', False),
    'timediff_gte': (_timedelta_resolver, 'gte', False),
    'timediff_lte': (_timedelta_resolver, 'lte', False),
    'timedelta_gt': (_timedelta_resolver, 'gt', False),
    'timedelta_lt': (_timedelta_resolver, 'lt', False),
    'timedelta_gte': (_timedelta_resolver, 'gte', False),
    'timedelta_lte': (_timedelta_resolver, 'lte', False),
}
