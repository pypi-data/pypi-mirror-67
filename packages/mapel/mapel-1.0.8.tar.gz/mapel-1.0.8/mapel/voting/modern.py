        ...
        for k in range(model.num_families):
            for _ in range(model.families[k].size):
                ...
                if coloring == "intervals":
                    ...
                    shade = 1
                ax.scatter(model.points[model.order[ctr]][0], model.points[model.order[ctr]][1],
...
def prepare_approx_cc_order(exp_name, metric="positionwise"):
    ...
    file_name = os.path.join(os.getcwd(), "experiments", str(exp_name), "results", "orders", str(metric) + "_approx_cc.txt")
    file_ = open(file_name, 'r')
