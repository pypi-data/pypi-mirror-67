...
## Example use
````python
from data_catalog_dcat_validator.models.dataset import Dataset
...
errors = Dataset(metadata).validate()
````
Output:
````bash
Missing recommended props(s): ['availability', 'license']
Missing recommended props(s): ['contactPoint', 'spatial', 'temporal', 'theme']
````