...
class Graph:
        ...
        while len(checked) < edges_num:
            ...
            while path and cross and len(checked) + len(path) < edges_num:
                ...
                if b:
                    break
                if tries > 1000:
                    #TODO Temporary debug info - by PR
                    print('REACHED 1000 TRIES in generate_orientation!!!!')
                    print('edges: ' + str(edges))
                    print('cross: ' + str(cross))
                    print('crossing_edges: ' + str(crossing_edges))
                    print('c: ' + str(c))
                    print('checked: ' + str(checked))
                    print('path: ' + str(path))
                    print('paths: ' + str(paths))
                    print('vertex_edges: ' + str(vertex_edges))
                    break
            if path:
                paths.append(path)
    ...
    def parse_substructure(self, arcs, abstract=False, output_type=OutputType.PDcode, output='list',
    ...
    def find_links(self, output_type=OutputType.PDcode, arc_bridges=1, output='list', file_prefix='link',
        ...
        for pot_link in combinations(self.find_loops(output_type=o_type, arc_bridges=arc_bridges, output='generator'),
            ...
            result = self.parse_substructure(arcs, abstract=abstract, output_type=output_type, output=output,
                                             file_prefix=file_prefix, folder_prefix=folder_prefix,
                                             loop_nr=len(links), arc_bridges=arc_bridges)
            if result:
                if output == 'generator':
                    yield result
                else:
    ...
    def find_handcuffs(self, output_type=OutputType.PDcode, arc_bridges=1, output='list', file_prefix='handcuff',
                       folder_prefix=''):
        graph, abstract, vertices = self.decide_finding_type(output_type)
        found = []
        handcuffs = []
                    ...
                    for arc3 in find_path(v2, v2, graph):
                        ...
                        if edges not in found and set(arc1) & set(arc2) == {v1} and set(arc2) & set(arc3) == {v2} and \
                            ...
                            result = self.parse_substructure(arcs, abstract=abstract, output_type=output_type,
                                                             output=output, file_prefix=file_prefix,
                                                             folder_prefix=folder_prefix, loop_nr=len(handcuffs),
    ...
    def find_thetas(self, output_type=OutputType.PDcode, arc_bridges=1, output='list', file_prefix='theta',
                    folder_prefix=''):
        graph, abstract, vertices = self.decide_finding_type(output_type)
        found = []
        thetas = []
        ...
        for v1, v2 in combinations(vertices, 2):
            for arc1, arc2, arc3 in combinations([path for path in find_path(v1, v2, graph)], 3):
                ...
                if edges not in found and set(arc1) & set(arc2) <= verts and set(arc2) & set(arc3) <= verts and\
                    ...
                    result = self.parse_substructure(arcs, abstract=abstract, output_type=output_type, output=output,
                                                     file_prefix=file_prefix, folder_prefix=folder_prefix,
                                                     loop_nr=len(thetas), arc_bridges=arc_bridges)
                    if result:
                        if output == 'generator':
                            yield result
                        else:
