...
def test_ansible_inventory():
    i = ansible_utils.ansible_inventory()
    assert '_meta' in i


def test_get_groups():
    i = ansible_utils.get_groups()
...
def test_flat_inventory():
    ...
    assert sorted(ansible_utils._flat_inventory(inventory, inventory['group1'])) == expected
    expected = ['host70', 'host71']
    assert sorted(ansible_utils._flat_inventory(inventory, inventory['group7'])) == expected
