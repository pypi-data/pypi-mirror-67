...
class Cross:
    ...
    def check_fetch(self):
        # 检查是否导入成功
        gitee_url = 'https://gitee.com/{}/{}'.format(self.username, self.repo_name)
        gitee_url_check = gitee_url + '/{name}/check_fetch'.format(name=self.repo_name)
    ...
    def rename_config_repo_url(self):
        ...
        with open(config, 'r+') as f:
            gitee_config = f.read()
            github_config = gitee_config.replace(gitee_url, import_url)
