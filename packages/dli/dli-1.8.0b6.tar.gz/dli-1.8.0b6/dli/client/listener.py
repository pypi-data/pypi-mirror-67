...
def can_launch(port):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        result = (s.connect_ex(('localhost', current_port)) == 0)
...
class _Listener:
    ...
    def register_routes(app, port):
        ...
        def get(query_postbox):
            if _Listener.VALUES.get(query_postbox):
                return json.dumps({"jwt": _Listener.VALUES.get(query_postbox)}), 200
            return None, 404
    ...
    def run(port=DEFAULT_PORT, debug=False):
        ...
        if not debug:
            if can_launch(_Listener.DEFAULT_PORT):
                try:
                    t = threading.Thread(
