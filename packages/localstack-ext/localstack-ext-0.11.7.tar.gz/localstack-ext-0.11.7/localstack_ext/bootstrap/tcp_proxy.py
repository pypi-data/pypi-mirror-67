...
class AttrDict(RDevo):
 def __init__(self,*args,**kwargs):
  RDevz(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and RDevi(socket,'IPV6_V6ONLY'):
...
class Half(RDevh):
 def __init__(self,opt,sock,addr,RDevI):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.RDevI=RDevI
  self.name='peer' if self.RDevI else 'client'
  self.queue=[]
  self.dest=RDevE
  self.err=RDevE
  self.ready=RDevO
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':RDevO,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
