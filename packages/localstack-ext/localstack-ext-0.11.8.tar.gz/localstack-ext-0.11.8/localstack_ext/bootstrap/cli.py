...
def import_auth():
 try:
  ...
  raise bXHeG('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=bXHei)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=bXHeE)
def get_params(args,names=[],mandatory=bXHei):
 result=()
 for name in names:
  value=bXHeV
  if name in args:
  ...
  if not value and mandatory:
   raise bXHeG('Please provide %s=... parameter'%name)
  result+=(value)
 if bXHeQ(result)==1:
  return result[0]
 return result
