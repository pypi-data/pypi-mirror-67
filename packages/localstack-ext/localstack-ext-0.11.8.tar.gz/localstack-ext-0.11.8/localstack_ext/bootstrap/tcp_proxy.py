...
class AttrDict(pKJgr):
 def __init__(self,*args,**kwargs):
  pKJga(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and pKJgD(socket,'IPV6_V6ONLY'):
...
class Half(pKJgb):
 def __init__(self,opt,sock,addr,pKJgR):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.pKJgR=pKJgR
  self.name='peer' if self.pKJgR else 'client'
  self.queue=[]
  self.dest=pKJgS
  self.err=pKJgS
  self.ready=pKJgl
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':pKJgl,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
