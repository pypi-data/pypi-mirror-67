...
def import_auth():
 try:
  ...
  raise lQgjE('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=lQgjy)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=lQgjk)
def get_params(args,names=[],mandatory=lQgjy):
 result=()
 for name in names:
  value=lQgjf
  if name in args:
  ...
  if not value and mandatory:
   raise lQgjE('Please provide %s=... parameter'%name)
  result+=(value)
 if lQgjF(result)==1:
  return result[0]
 return result
