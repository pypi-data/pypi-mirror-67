...
def import_auth():
 try:
  ...
  raise DuLCQ('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=DuLCe)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=DuLCI)
def get_params(args,names=[],mandatory=DuLCe):
 result=()
 for name in names:
  value=DuLCz
  if name in args:
  ...
  if not value and mandatory:
   raise DuLCQ('Please provide %s=... parameter'%name)
  result+=(value)
 if DuLCp(result)==1:
  return result[0]
 return result
