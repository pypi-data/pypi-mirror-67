...
def import_auth():
 try:
  ...
  raise AqjLH('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=AqjLY)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=AqjLt)
def get_params(args,names=[],mandatory=AqjLY):
 result=()
 for name in names:
  value=AqjLP
  if name in args:
  ...
  if not value and mandatory:
   raise AqjLH('Please provide %s=... parameter'%name)
  result+=(value)
 if AqjLU(result)==1:
  return result[0]
 return result
