...
class AttrDict(eXKIc):
 def __init__(self,*args,**kwargs):
  eXKIB(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and eXKIC(socket,'IPV6_V6ONLY'):
...
class Half(eXKIw):
 def __init__(self,opt,sock,addr,eXKIs):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.eXKIs=eXKIs
  self.name='peer' if self.eXKIs else 'client'
  self.queue=[]
  self.dest=eXKIR
  self.err=eXKIR
  self.ready=eXKIU
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':eXKIU,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
