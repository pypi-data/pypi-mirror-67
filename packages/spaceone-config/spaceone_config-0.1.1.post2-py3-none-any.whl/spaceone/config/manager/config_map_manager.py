...
class ConfigMapManager(BaseManager):
    ...
    def update_config_map(self, params):
        config_map_vo: ConfigMap = self.get_config_map(params['config_map_id'], params['domain_id'])
        return self.update_config_map_by_vo(params, config_map_vo)

    def update_config_map_by_vo(self, params, config_map_vo):
        def _rollback(old_data):
            _LOGGER.info(f'[update_config_map_by_vo._rollback] Revert Data : '
                         f'{old_data["config_map_id"]}')
    ...
    def delete_config_map(self, config_map_id, domain_id):
        config_map_vo: ConfigMap = self.get_config_map(config_map_id, domain_id)
        config_map_vo.delete()

    def get_config_map(self, config_map_id, domain_id, only=None):
        return self.config_map_model.get(config_map_id=config_map_id, domain_id=domain_id, only=only)
