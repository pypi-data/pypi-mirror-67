...
class UnauthorizedError(Exception):
    """ Custom exception that is raised when the provided deploy key is invalid.
...
def store_rho_model(model: Type[RhoModel],
    ...
    Args:
        ...
        deploy_key (str, optional): Sermos deploy key.  If not passed here it
            must be set in the environment as `SERMOS_DEPLOY_KEY`
    ...
    Example::
        ...
        store_rho_model(model=my_rho_model)  # deploy key set in environment
    """
...
def load_rho_model(model_name: str,
    ...
    Args:
        ...
        delimiter_pattern (str, optional): delimiter between the model name and
            version.
        deploy_key (str, optional): Sermos deploy key.  If not passed here it must
            be set in the environment as `SERMOS_DEPLOY_KEY`
    ...
    Example::
        ...
        # deploy key set in the environment
...
def _search_model_api_response(deploy_key: str,
...
def search_rho_model(model_name: str,
    ...
    resp = _search_model_api_response(
        deploy_key, model_name, version_pattern, search_model_endpoint
    )
