...
class Random(random.Random):
    ...
    def from_data(cls, data):
        key = [data[k] for k in Build.mandatory_keys if k != "kconfig"]
        key += [data[k] for k in Build.xor_keys if k in data]
        key.append(":".join(sorted(data["kconfig"])))
        seed = hash(frozenset(key))
        return cls(seed)

    def get_build_key(self):
        return "".join(self.choice(self.build_key_alphabet) for i in range(22))

    def get_git_sha(self):
        return "".join(self.choice(self.git_sha_alphabet) for i in range(22))

    def get_chance(self, p):
        return random.randint(1, 100) < p

    def get_build_status(self):
        return self.get_chance(self.build_failure_rate) and "fail" or "pass"

    def get_warnings(self):
        return int(self.expovariate(1.5))

    def get_errors(self):
        return int(self.expovariate(2))

    def get_phrase(self):
        return self.choice(phrases)
    ...
    def generate_api_failures(cls, f):
        ...
        def wrapper(*args, **kwargs):
            if cls().get_chance(cls.api_failure_rate):
...
class Build:
    ...
    mandatory_keys = ["git_repo", "target_arch", "toolchain", "kconfig"]
...
def run(build_failure_rate, api_failure_rate, port):
    Random.build_failure_rate = build_failure_rate
    Random.api_failure_rate = api_failure_rate
    app.run(port=port)
