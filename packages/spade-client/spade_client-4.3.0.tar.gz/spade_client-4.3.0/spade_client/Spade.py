...
class Spade(object):
    ...
    def get_time_stampers(self, stamper):
        uri = stamper.find('uri').text
        print("Time Stamper URI : " + uri )
        r = self.session.get(uri, headers=HEADERS)
        _check_status(uri, r, 200)
        timestamps = ET.fromstring(r.text)
        self._pretty_print(uri,ET.tostring(timestamps))
        digests = timestamps.findall('digest')
        if 0 == len(digests):
            Display.digest(timestamps, '')
        else:
