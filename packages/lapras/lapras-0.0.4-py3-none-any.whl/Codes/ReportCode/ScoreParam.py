...
from Settings import col_head,woe_file_output
from DealConfig import params, A, B, theta, woe_all, cut_points_step, assist_num, bond_list, eps, get_GBK_Name
from DealConfig import makeModelDataFillnaFileName, scoreCardDataFileName, comParamCorrFileName, checkDataFileName,\
...
if __name__ == '__main__':
    ...
    # 保存入模变量 变量相似性 得分结果
    get_score_csv(makeModelDataFillnaFileName, scoreCardDataFileName, comParamCorrFileName, checkDataFileName, save_file=False)
    # 得到计分区间
    base_score_min, base_score_max = get_score_max_min(A, B, theta, woe_all)
    # 自动划分区间
    break_points = get_score_bond(cut_points_step, assist_num=assist_num, minpoint=base_score_min, maxpoint=base_score_max)
    print (break_points)
