...
class ParameterLookup(object):
    ...
    def __init__(self, params):
        ...
        for param in params:
            ...
            dict_used = self.opts if isinstance(param, click.Option) else self.args
            for opt in param.opts + param.secondary_opts:
                dict_used[opt] = param
        self.num_arguments = len(self.args)
    ...
    def parse(self):
        ...
        for key, value in cmd_opts.items():
            ...
            if isinstance(param, click.Argument):
                arguments[param.name] = self._parse_argument(value, param)
...
class CmdFileMixin(object):
    ...
    def add_cmd_file_options(self):
        if self.params:
            click.option(
                CMD_FILE_PARAM_NAME,
                type=click.File(),
                help="A path to a YAML or JSON file to load the arguments from",
            )(self)
            click.option(
                WRITE_CMD_FILE_PARAM_NAME,
                type=click.Path(dir_okay=False, writable=True, readable=False),
                help="Save the arguments to this command to a file",
            )(self)
    ...
    def write_cmd_file(self, ctx):
        write_file = ctx.params.get(WRITE_CMD_FILE_PARAM, None)
        if write_file:
    ...
    def write_params(self, ctx, write_file):
        ...
        for name, value in ctx.params.items():
            if name in (CMD_FILE_PARAM, WRITE_CMD_FILE_PARAM):
                continue
    ...
    @staticmethod
    def strip_cmd_file_params(params):
        # Removes the --from-file and --save-command params so invoked functions do not need to
        # accept them.
        file_path = params.pop(CMD_FILE_PARAM, None)
        write_file_path = params.pop(WRITE_CMD_FILE_PARAM, None)
        return file_path, write_file_path
...
class CommandWithCmdFile(CmdFileMixin, click.Command):
    ...
    def invoke(self, ctx):
        self.strip_cmd_file_params(ctx.params)
...
class GroupWithCmdFile(CmdFileMixin, click.Group):
    ...
    def invoke(self, ctx):
        self.strip_cmd_file_params(ctx.params)
        return super(GroupWithCmdFile, self).invoke(ctx)
