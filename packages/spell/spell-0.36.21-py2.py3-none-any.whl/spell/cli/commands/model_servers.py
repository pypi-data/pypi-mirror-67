...
from spell.api.models import SpellModelServerRequest
...
@dependency_params(include_python_version=False, include_conda=False)
@workspace_spec_params
@description_param
@cli_params
@click.pass_context
...
def get_name_and_tag(specifier):
    ...
    if len(name_tag) != 2:
        raise ExitException(
            "Invalid name {}. Expected <model_name>:<tag> format.".format(specifier)
        )
