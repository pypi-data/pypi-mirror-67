...
from spell.cli.utils import is_installed, cluster_utils, prettify_time, tabulate_rows, HiddenOption
...
def serving_group(ctx):
    ...
    # TODO(peter): List subcommands
    print("List serving groups")
...
@click.option(
    "--name", help="Name of the serving group",
)
@click.option(
    "--instance-type", help="Instance type to use for serving group nodes",
)
@click.option(
    "--accelerators", help="(GCP only) Accelerators to attach to nodes",
)
@click.option(
...
def serving_group_add(
    ...
    with api_client_exception_handler():
        # TODO(peter): Make this an upsert for serving groups that haven't successfully been initialized yet
        serving_group = spell_client.register_serving_group(
            cluster_name,
            name,
            instance_type,
            accelerators,
    ...
    if cluster["cloud_provider"] == "AWS":
        eks_add_nodegroup(serving_group.config, profile)
    elif cluster["cloud_provider"] == "GCP":
        print("This functionality is not yet implemented for GCP")

    with api_client_exception_handler():
        spell_client.mark_serving_group_created(cluster_name, serving_group.id)
