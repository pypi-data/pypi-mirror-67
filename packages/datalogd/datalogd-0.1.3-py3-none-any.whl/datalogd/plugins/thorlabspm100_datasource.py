...
try:
    ...
    log.error("ThorlabsPM100 and/or visa module not found. Install it with \"pip3 install pyvisa ThorlabsPM100\" or similar. A VISA backend must also be present, use pyvisa-py if the native NI libraries are not installed.")
else:
    ...
    class ThorlabsPM100DataSource(DataSource):
        ...
        """
        def __init__(self, sinks=[], serialnumber=None, interval=1.0):
            ...
            res = self.rm.list_resources("USB0::4883::32888::{}::0::INSTR".format(serialnumber if serialnumber else "?*"))
            if len(res) == 0:
