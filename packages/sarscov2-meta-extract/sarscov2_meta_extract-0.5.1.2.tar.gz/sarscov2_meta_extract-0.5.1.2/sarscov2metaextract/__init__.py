...
def convertor(align, meta_source)
# Create dictionary of duplicate sequences and list of all accession IDs
	...
	if meta_source == 'ncbi':
		...
		for item in data['genbank-sequences']:
			...
			if item['accession'] in all_acc:
				acc = item['accession']
				date = item['collection_date'].replace('-', '')
				if len(date) == 6:
					if acc == 'NC_045512':
						date = '20200117'
					else:
	...
	for key in meta_multi.keys():
		dups[multi_seqs[key][0]] = {}
		for acc in meta_multi[key]:
			...
			dups[meta_multi[key][0]][str(all_acc.index(acc))] = acc+'_'+loc+'_'+meta[acc]['collected']+'_null'
	...
	for record in records:
		if record.id in meta_acc or record.id in meta_multi.keys():
