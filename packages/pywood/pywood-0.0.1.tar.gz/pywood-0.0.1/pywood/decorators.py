...
def event(event_cls: Type[BaseEvent]):
    ...
    def decorator(method):
        ...
        @wraps(method)
        def wrapper(state, update, data):
            method_name = method.__name__
            if event_cls.happened(update):
                state.before_handling(method_name, event_cls)
                method(state, update, data)
                state.after_handling(method_name, event_cls)
                raise EventHandled
    ...
    def decorator(method):
        ...
        def wrapper(state, update, data):
            for event_cls in event_classes:
                if event_cls.happened(update):
                    state.before_handling(method_name, event_cls)
                    method(state, update, event_cls, data)
                    state.after_handling(method_name, event_cls)
                    raise EventHandled
...
def no_events(method):
    ...
    @wraps(method)
    def wrapper(state, update, data):
        state.before_handling(method_name, None)
        method(state, update, data)
        state.after_handling(method_name, None)
