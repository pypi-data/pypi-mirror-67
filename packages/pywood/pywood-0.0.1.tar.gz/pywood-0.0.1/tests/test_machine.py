...
class TestMachine(unittest.TestCase):
    def test_method__get_state_from_state_name(self):
        ...
        machine = Machine(current_state_getter=lambda update: State3,
                          states=[State1, State2, State3])

        self.assertIs(machine._get_state_from_state_name('State2'), State2)
    def test_method__get_current_state(self):
        ...
        machine = Machine(current_state_getter=lambda update: State2,
                          states=[State1, State2, State3])
        self.assertIs(machine._get_current_state(state='State2'), State2)
        self.assertIs(machine._get_current_state(state=State2), State2)
    def test_method_process(self):
        ...
        class TestState1(BaseState):
            ...
            @events(Event5)
            def method5(self, update, event_cls, data):
                state1_method5_mock()
            ...
            def after_handling(self, method_name, event_cls):
                after_handling_mock_state1(method_name, event_cls)

            def before_handling(self, method_name, event_cls):
                before_handling_mock_state1(method_name, event_cls)

        class TestState2(BaseState):
            @event(Event5)
            def method1(self, update, data):
                state2_method1_mock()
            ...
            def method4(self, update, event_cls, data):
                state2_method4_mock()
            ...
            def after_handling(self, method_name, event_cls):
                after_handling_mock_state2(method_name, event_cls)

            def before_handling(self, method_name, event_cls):
                before_handling_mock_state2(method_name, event_cls)

        class TestState3(BaseState):
            @events(Event4, Event5, Event6)
            def method1(self, update, event_cls, data):
                state3_method1_mock()
            ...
            def after_handling(self, method_name, event_cls):
                after_handling_mock_state3(method_name, event_cls)

            def before_handling(self, method_name, event_cls):
                before_handling_mock_state3(method_name, event_cls)
        ...
        class TestState4(BaseState):
            ...
            def after_handling(self, method_name, event_cls):
                after_handling_mock_state4(method_name, event_cls)

            def before_handling(self, method_name, event_cls):
                before_handling_mock_state4(method_name, event_cls)


        machine = Machine(current_state_getter=lambda update: TestState1,
                          states=[TestState1, TestState2, TestState3])
        machine.process(update='event5')
        ...
        before_handling_mock_state1.assert_called_once_with('method5', Event5)
        after_handling_mock_state1.assert_called_once_with('method5', Event5)


        machine = Machine(current_state_getter=lambda update: TestState2,
                          states=[TestState1, TestState2, TestState3])
        machine.process(update='event6')
        ...
        before_handling_mock_state2.assert_called_once_with('method2', None)
        after_handling_mock_state2.assert_called_once_with('method2', None)


        machine = Machine(current_state_getter=lambda update: TestState3,
                          states=[TestState1, TestState2, TestState3])
        machine.process(update='event5')
        ...
        before_handling_mock_state3.assert_called_once_with('method1', Event5)
        after_handling_mock_state3.assert_called_once_with('method1', Event5)


        machine = Machine(current_state_getter=lambda update: TestState4,
                          states=[TestState1, TestState2, TestState3])
        machine.process(update='event6')
    ...
    def test_state_has_passed_attrs(self):
        class State1(BaseState):
            pass
        class State2(BaseState):
            pass
    ...
    def test_event_passed_data(self):
        class State(BaseState):
            def handler(self, update, data):
                pass
            
        mock = Mock(__name__='handler')
        State.handler = event(EventTrue1)(mock)
        machine = Machine(current_state_getter=lambda update: State,
                          states=[State])
        machine.process('update', data={'token': '123'})
        mock.called_once_with(ANY, 'update', {'token': '123'})



    def test_events_passed_data(self):
        class State(BaseState):
            def handler(self, update, data):
                pass

        mock = Mock(__name__='handler')
        State.handler = events(EventFalse1, EventFalse2, EventTrue1)(mock)
        machine = Machine(current_state_getter=lambda update: State,
                          states=[State])
        machine.process('update', data={'token': '123'})
        mock.called_once_with(ANY, 'update', EventTrue1, {'token': '123'})

    def test_no_events_passed_data(self):
        class State(BaseState):
            def handler(self, update, data):
                pass

        mock = Mock(__name__='handler')
        State.handler = no_events(mock)
        machine = Machine(current_state_getter=lambda update: State,
                          states=[State])
        machine.process('update', data={'token': '123'})
        mock.called_once_with(ANY, 'update', {'token': '123'})
