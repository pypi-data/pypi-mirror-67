...
class Peer:
    ...
    def _send(self, peer: Connection, packet: protocol.Packet) -> Tuple[int, Any]:
        if peer.lock.acquire():
            try:
                ...
                peer.socket.send(packet.raw())

                data = bytearray()

                try:
                    while True:
                        data_ = peer.socket.recv(self.buffer)
                        if data_:
                            data.extend(data_)
                            print(len(data))
                        else:
                    ...
                    if e.errno is not errno.EWOULDBLOCK:
                        raise e
    ...
    def listener_loop(self):
        while self._state > 0:
            ...
            readable, writeable, exceptional = select.select(readers, writers, readers, .5)
            ...
            for i in readable:
                if i is self._server:
                    ...
                    if i.lock.acquire(False):
                        ...
                        try:
                            while True:
                                data_ = i.socket.recv(self.buffer)
                                print(len(data_))
                                if data_:
                                    data.extend(data_)
                            ...
                            if e.errno is errno.ECONNRESET:
                                data = None
                            elif e.errno is errno.EWOULDBLOCK:
                                pass
                            else:
                                raise e
