...
class TempFileWriter(object):
    """Writer for temporary files that can be consumed by
       the autoscaler or rsync."""
...
def ray_cli(ctx: Any) -> None:
    ...
    old_callback = copy.deepcopy(subcommand.callback)
    ...
    def autoscaler_callback(*args: Any, **kwargs: Any) -> None:
        ...
        session = get_project_session(project_id, session_name)
        resp = send_json_request("session_describe", {"session_id": session["id"]})
        snapshot_uuid = resp["applied_snapshots"][-1]["snapshot_uuid"]
        description = describe_snapshot(snapshot_uuid)
        ...
        if not os.path.exists(key_path):
            with open(key_path, "w") as f:
                ...
                f.write(resp["private_key"])
        # Store key in autoscaler cluster config.
        cluster_config["auth"]["ssh_private_key"] = key_path
        if "head_node" not in cluster_config:
            cluster_config["head_node"] = {}
        if "worker_nodes" not in cluster_config:
            cluster_config["worker_nodes"] = {}
        cluster_config["head_node"]["KeyName"] = key_name
        cluster_config["worker_nodes"]["KeyName"] = key_name
        with TempFileWriter(json.dumps(cluster_config)) as cluster_yaml_path:
            kwargs["cluster_config_file"] = cluster_yaml_path
