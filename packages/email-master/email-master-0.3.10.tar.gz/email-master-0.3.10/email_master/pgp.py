    ...
    def unlock(self, msg_as_string):
        ...
        if is_encrypted:
            with self.priv_key.unlock(self.key_pw) as unlocked_key:
                ...
                email_data = EmailMasterParser(base64_encode_bytes(str(decrypted.msg).encode())).parse()
        else:
            email_data = EmailMasterParser(base64_encode_bytes(str(msg_as_string).encode())).parse()

        email_data["message_type"] = message_type
        return email_data
...
class CustomMIMEWrapper(object):
    ...
    def _py2_clean_msg(self):
        copied_msg = copy.deepcopy(self.msg)
        copied_msg._payload = "\n".join([str(p) for p in self.msg.get_payload()])

        if len(self.msg.get_payload()) == 1:  # No attachments
            copied_msg["Content-Type"] = "message/rfc822"
            copied_msg["Content-Transfer-Encoding"] = "8bit"
