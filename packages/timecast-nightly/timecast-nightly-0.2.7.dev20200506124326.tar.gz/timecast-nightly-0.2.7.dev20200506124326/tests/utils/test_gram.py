...
def test_gram_update(X_dim, Y_dim, n):
    ...
    onp.testing.assert_array_almost_equal(
        gram.matrix(normalize=False, fit_intercept=False), X.T @ Y
    )
...
@pytest.mark.parametrize("input_dim", [None, 1, 4])
def test_gram_intercept_xtx(X_dim, n, normalize, input_dim):
    ...
    X = jnp.hstack((jnp.ones((n, num_rc)), X))

    onp.testing.assert_array_almost_equal(
        gram.matrix(normalize=normalize, fit_intercept=True, input_dim=input_dim),
        X.T @ X,
        decimal=1,
    )
...
@pytest.mark.parametrize("normalize", [True, False])
@pytest.mark.parametrize("input_dim", [None, 1, 4])
def test_gram_intercept_xty(X_dim, Y_dim, n, normalize, input_dim):
...
def test_gram_projection_no_projection():
    ...
    XTX = OnlineGram(10)
    XTX.update(X)
    onp.testing.assert_array_almost_equal(X.T @ X, XTX.project())
