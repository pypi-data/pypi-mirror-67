...
def _form_constraints(
    ...
    R = onp.zeros((num_constraints, input_dim * history_len))
...
def _fit_constrained(beta, inv, R, r):
    """Fit constrained"""
    return beta - inv @ (R.T @ jnp.linalg.inv(R @ (inv @ R.T))) @ (R @ beta - r)
...
def fit_gram(
    ...
    if XTX.observations <= feature_dim:
        raise ValueError("Underdetermined systems not currently supported")
    if input_dim is not None and feature_dim % input_dim != 0:
...
def compute_gram(
    ...
    for X, Y, _ in data:
        ...
        history = historify(X, num_histories=num_histories, history_len=history_len, offset=0)
