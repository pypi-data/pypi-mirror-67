...
# location.
def peak_values(column_sum,row_sum,n_columns,n_rows,image, freeze_heat):
    '''
    Function to find peaks from the column_sum and row_sum arrays
    and return a dataframe with sample locations.
    Use this when the wells are at a lower temperature when compared to the 
    plate in 'img_eq'
...
##### Wrapping Function ######
def pixel_temp(frames,n_frames,n_columns,n_rows,freeze_heat):
    ''' 
    ...
    # Function to find peaks from the column_sum and row_sum arrays
    r_peaks, c_peaks = peak_values(column_sum,row_sum,n_columns,n_rows,img_eq,freeze_heat)
