...
class V1Dag(BaseConfig, polyaxon_sdk.V1Dag):
    ...
    def _get_op_upstream(self, op, params, upstream: Set) -> Set:
        if not params:
            return upstream
        ...
        for param in params:
            param_spec = params[param].get_spec(
    ...
    def _process_op(self, op):
        ...
        upstream = self._get_op_upstream(op=op, params=op.params, upstream=upstream)
        upstream = self._get_op_upstream(op=op, params=op.context, upstream=upstream)
