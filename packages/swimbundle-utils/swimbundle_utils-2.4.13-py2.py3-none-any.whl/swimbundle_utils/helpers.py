    ...
    def is_datetime(val):
        if val.startswith("{time.now():") and val.endswith("}"):
            ...
            if not is_add and not time_val.startswith("-"):
                raise SwimlaneIntegrationException("Time value in QueryString must contain a + or - modifier!")
            if is_add:
                fn_name = "add"
            else:
                fn_name = "subtract"
            ...
            if not match:
                raise SwimlaneIntegrationException("Invalid time value! must be in format <int><time value>. See a README for examples.")
