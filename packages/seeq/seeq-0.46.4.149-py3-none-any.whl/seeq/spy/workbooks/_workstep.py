...
class AnalysisWorkstep(Workstep):
    def __init__(self, worksheet, definition=None):
        ...
        if self.display_range is None and self.investigate_range is None:
            self._set_display_range({'Start': datetime.datetime.now() - pd.Timedelta(days=1), 'End': datetime.datetime.now()},
                                    check_investigate=False)
            self._set_investigate_range({'Start': datetime.datetime.now() - pd.Timedelta(days=7), 'End': datetime.datetime.now()},
                                        check_display=False)
        # initialize the view
