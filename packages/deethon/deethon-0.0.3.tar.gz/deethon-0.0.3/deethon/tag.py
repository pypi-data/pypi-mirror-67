...
def tag(file_path, track):
    ...
    if ext == ".mp3":
        ...
        tag["TIT2"] = TIT2(encoding=3, text=track.title)
        ...
        tag["TRCK"] = TRCK(encoding=3, text=f"{track.number}/{track.album.total_tracks}")
        ...
        tag["APIC"] = APIC(
            encoding=3, mime="image/jpeg", type=3, desc="Cover", data=track.album.cover_xl,
        )
        tag.save(file_path)
