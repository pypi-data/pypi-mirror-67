...
ANALYSIS_URL = 'https://x-developer.cn/analysis/update/'
            ...
            if res['status'] == 'success':
                ...
                if flag:
                    ...
                    if response['status'] == 'success':
                        ...
                        print('分析结果：失败。')
                        message = '失败原因：' + response['message'].encode('utf-8')
                        print(message)
                        sys.exit(1)
                    for log in data_sources:
                        log_path = data_sources.get(log)
                    ...
                    print('已更新'+log.encode('utf-8')+'日志最新提交时间。')
            else:
                print('采集器同步结果：失败。')
                print('错误原因：' + res['status'].encode('utf-8'))
                sys.exit(1)
            sys.exit(0)
        else:
            parser.print_help()
