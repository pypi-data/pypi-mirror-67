...
return series capacitance Cs (farad) if defines"""
        if x is None or x >= 0:
            return None
        return -1/(x*w)
    ...
    def get_series_inductance(x, w):
        """x : reactance (ohm)
w : angular frequency (rad/s)
return series inductance Ls (henry) if defines"""
        if x is None or x <= 0:
            return None
        return x/w
...
w : angular frequency (rad/s)
return parallel capacitance Cp (farad) if defines"""
        if b is None or b <= 0:
            return None
        return b/w
    ...
    def get_parallel_inductance(b, w):
        """b : susceptance (siemens)
w : angular frequency (rad/s)
return parallel inductance Lp (henry) if defines"""
        if b is None or b >= 0:
            return None
        return -1/(b*w)
...
>>> Zc.bode(yscale='log', title='Zc frequency response')
>>> Yc = 1/Zc
>>> Yc.bode(yscale='log', title='Yc frequency response')
>>> show()
"""
        if not _matplotlib_available:
            print("You should install matplotlib to draw Bode plot")
...
"""
        ...
        if "r" in kwargs:
            ...
            self.fw = Wfunction(lambda w: 1/(1j*c*w))
        elif "l" in kwargs:
            ...
            self.fw = Wfunction(lambda w: 1j*l*w)
        elif "fw" in kwargs:
...
"""
    ...
    def properties(self, value, unit='Hz', message=''):
        "Return and displays the complex impedance properties"
...
""".format(f, w, self(f, 'Hz'),
    ...
    def __str__(self):
        return "Complex impedance (Ω) : {:g} \
@ 1 kHz".format(self(value=1000, unit='Hz'))
...
"""
        ...
        if "r" in kwargs:
            ...
            if self.r <= 0:
                raise ValueError("positive resistance expected")
            self.fw = Wfunction(lambda w: 1/self.r)
        elif "c" in kwargs:
            if not isinstance(kwargs["c"], (int, float)):
                raise TypeError("number expected")
            self.c = kwargs["c"]
            if self.c <= 0:
                raise ValueError("positive capacitance expected")
            self.fw = Wfunction(lambda w: 1j*self.c*w)
        elif "l" in kwargs:
            if not isinstance(kwargs["l"], (int, float)):
                raise TypeError("number expected")
            self.l = kwargs["l"]
            if self.l <= 0:
                raise ValueError("positive inductance expected")
            self.fw = Wfunction(lambda w: 1/(1j*self.l*w))
        elif "fw" in kwargs:
            if isinstance(kwargs["fw"], Wfunction):
                self.fw = kwargs["fw"]
            elif inspect.isfunction(kwargs["fw"]):
...
"""
    ...
    def properties(self, value, unit='Hz', message=''):
        "Return and displays the complex admittance properties"
...
""".format(f, w, self(f, 'Hz'),
    ...
    def __str__(self):
        return "Complex admittance (S) : \
{:g} @ 1 kHz".format(self(value=1000, unit='Hz'))
...
"""
        ...
        if magnitude_unit == 'db':
            ax1.set_ylabel('{} [{}]'.format(magnitude_label, self.db_unit), color='b')
        elif magnitude_unit == 'default':
            ax1.set_ylabel('{} [{}]'.format(magnitude_label, self.unit), color='b')
...
"""
        ...
        if "fw" in kwargs:
            ...
            if not isinstance(ratio, (int, float, complex)):
                raise TypeError("number expected")
            self.fw = Wfunction(lambda w: ratio)
...
"""
    ...
    def properties(self, value, unit='Hz', message=''):
        "Return and displays the Ratio or transfer function properties"
        # f : frequency (Hz) ; w : angular frequency (rad/s)
        (f, w) = getfrequency(value, unit=unit)
        affichage = '' if message is '' else message + '\n'
        affichage += """Frequency (Hz) : {:g}
...
"""
        ...
        def H(w):   # w -> rad/s
            # numerator
            num = 0
            for i, coeff in enumerate(numerator):
                num += coeff*w**i
            # denominator
            den = 0
...
"""
        ...
        if "fw" in kwargs:
            ...
            if not isinstance(phase, (int, float)):
                raise TypeError("number expected")
            phase = math.radians(phase)  # degrees -> radians
            self.fw = Wfunction(lambda w: cmath.rect(rms, phase))
...
"""
    ...
    def properties(self, value, unit='Hz', message=''):
        "Return and displays the complex voltage properties"
        # f : frequency (Hz) ; w : angular frequency (rad/s)
        (f, w) = getfrequency(value=value, unit=unit)
...
"""
        ...
        if "fw" in kwargs:
            ...
            if not isinstance(phase, (int, float)):
                raise TypeError("number expected")
            phase = math.radians(phase)  # degree -> radian
            self.fw = Wfunction(lambda w: cmath.rect(rms, phase))
...
class Power(ElectricalQuantity):
    ...
    def __init__(self, value=1+0j, **kwargs):
...
"""
        ...
        if "fw" in kwargs:
            ...
            if not isinstance(value, (int, float, complex)):
                raise TypeError("number expected")
            self.fw = Wfunction(lambda w: value)
...
"""
    ...
    def properties(self, value, unit='Hz', message=''):
        "Return and displays the complex power properties"
        # f : frequency (Hz) ; w : angular frequency (rad/s)
        (f, w) = getfrequency(value=value, unit=unit)
