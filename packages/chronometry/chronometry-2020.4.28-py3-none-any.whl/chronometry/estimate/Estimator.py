...
from ravenclaw.preprocessing import Polynomial
...
class Estimator:
	...
	def measurements(self):
		...
		for measurement in measurements:
			if min_elapsed > 0:
				measurement._weight = measurement.elapsed_time / min_elapsed
			else:
	...
	def get_x_data(self, x, degree=None):
		...
		if degree > 1:
			...
			return polynomial.fit_transform(data=data)
	...
	def predict_data(self, data):
		...
		elapsed = self.model.predict(self.get_x_data(data))
	...
	def explore(self, timeout, num_rounds=1000, arguments=None, **kwargs):
		start_time = get_now()
		...
		while remaining_time > 0 and num_candidates > 0:
	...
	def plot(self, x):
		...
		for name, group in groups:
			ax.plot(group[x], group['time'], marker='.', linestyle='', ms=12, label=name)
		ax.legend()
		plt.show()
		return fig, ax
