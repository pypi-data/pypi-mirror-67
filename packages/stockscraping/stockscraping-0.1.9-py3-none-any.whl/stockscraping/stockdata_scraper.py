...
class StockDataScraper:
    def extract_stocktable_from_yahoo(self, stock_code, search_range=None, trim_unit='d'):
        ...
        if search_range is None:
            ...
            # dt_nowで比較しようとすると ValueError: day is out of range for month が出るので迂回した
            max_enddatetime = datetime(dt_now.year, dt_now.month, dt_now.day) 
            if max_enddatetime < datetime(end_year, end_month, end_day):
                #raise ValueError(f'検索範囲終了時点を未来にできない．{dt_now.year}/{dt_now.month}/{dt_now.day}以前に指定せよ！')
                pass
