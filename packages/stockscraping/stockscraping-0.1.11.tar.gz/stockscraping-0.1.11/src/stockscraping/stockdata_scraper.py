...
class StockDataScraper:
    def extract_stocktable_from_yahoo(self, stock_code, search_range=None, trim_unit='d'):
        ...
        if search_range is None:
            ...
            """
            # datetimeに入力して比較しようとすると ValueError: day is out of range for month が出るので迂回した
            end_datetime = datetime.strptime(f'{end_year}{end_month}{end_day}', '%Y%m%d')
            if dt_now < end_datetime:
