...
namespace pyxie
{
	...
	uint8_t* createTextImage(const char* word, const char* fontpath, int fontsize, int texw, int texh) {
		...
		int b_w = texw/4 * 4 + ((texw % 4) ? 4 : 0);
		int b_h = texh;
		int l_h = fontsize;

		/* create a bitmap for the phrase */
		unsigned char* bitmap = (unsigned char*)pyxie::PYXIE_MALLOC(b_h * b_w);
		memset(bitmap, 0, b_h * b_w);
	...
	void savePYXI(uint8_t* pixels, int w, int h, int format, char* path) {
		int bitsize = pyxieTexture::GetBitSize(format);
		if (bitsize == 0) return;
		uint8_t* ptr = (uint8_t*)PYXIE_MALLOC(w * h * bitsize + sizeof(ktxheader) + sizeof(uint32_t));
		...
		*blocksize = w * h * bitsize;
		...
		pyxieFios::Instance().Write(path, ptr, w * h * 4 + sizeof(ktxheader));
	}
