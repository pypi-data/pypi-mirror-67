...
class Drone:
    ...
    def _runEventHandler(self, dataType):
        if isinstance(dataType, DataType) and self._eventHandler.d[dataType] is not None and self._storage.d[
    ...
    def drone_default_color(self, r=-1, g=-1, b=-1, brightness=-1):
        try:
    ...
    def drone_default_pattern(self, r=-1, g=-1, b=-1, pattern=-1, interval=-1):
        try:
            self.send_led_process(DataType.LightDefault, LightModeDrone(pattern.value),
                                  r, g, b, interval)
        except:
            self._print_error(">>> you put wrong parameter")  # print error message

    def drone_led_off(self):
        self.send_led_process(DataType.LightMode, LightModeDrone(Mode.OFF.value), Colors.White, 0)
