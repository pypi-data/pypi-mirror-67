...
class postgresqlExecutor():
        ...
        try:
            ...
            if self.cursor.description != None:
                for lie in self.cursor.description:
                    lies.append(lie[0])
            datas = list(self.cursor.fetchall()) if ifdatas else None
            return [lies, datas]
    ...
    def createTable(self, mode_table, columns: list, **args):
        # CREATE TABLE mode_table_name (column_name column_type);  Create Table If Not Exists
        key = args.get('key', None)
        columnclasslist = args.get('columnclasslist', ["varchar(255)" for i in range(len(columns))])
        # Create Table If Not Exists
        noexistcreate = args.get('noexistcreate', False)
        try:
            assert len(columns) > 0 and len(columns) == len(columnclasslist), "数量有误！"
            sql = 'Create Table If Not Exists ' if noexistcreate else "CREATE TABLE "
            sql += mode_table + " ("
            sql += ",".join([(columns[i] + " " + columnclasslist[i]) for i in range(len(columns))])
            sql += " ,PRIMARY KEY(" + key + "))" if key != None else ")"
