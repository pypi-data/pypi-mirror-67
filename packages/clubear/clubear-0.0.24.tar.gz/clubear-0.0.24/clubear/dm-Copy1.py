...
'''
        ...
        for j in range(nfiles):
            ...
            for i in range(ss):
                gender='M'; 
                if random.random()>0.5: gender='F'
                age=int(20+50*random.random())
                height=np.round(1.5+0.5*random.random(),2)
                weight=int(100+100*random.random())
                region=chr(int(65+25*random.random()))
                company=chr(int(70+5*random.random()))+chr(int(65+5*random.random()))
                brand=chr(int(75+10*random.random()))+chr(int(80+10*random.random()))
                price=np.round(random.random()*100)
                logsales=2.0*random.random()+0.5*np.sqrt(age)+0.3*(gender=='F')
