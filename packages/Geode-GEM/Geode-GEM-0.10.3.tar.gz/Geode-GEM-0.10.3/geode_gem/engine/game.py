...
class Game(object):
    ...
    def __init_environment_variables(self, environment):
        """ Initialize Game environment variables from dictionnary strucure

        Parameters
        ----------
        environment : dict
            Environment variables dictionnary structure
        """
    ...
    def init_from_api(self, parent):
        """ Initialize Game attributes from specified database object

        Parameters
        ----------
        parent : gem.engine.lib.database.Database
            Database instance

        Returns
        -------
        bool
            True if the database contains the game, False otherwise
        """

        if parent is None:
            return False

        # Update environment variables from API
        self.__init_environment_variables(parent.environment)

        # Retrieve data from database
        data = parent.database.get("games", {"filename": self.__path.name})
        if data is None:
            return False
        ...
        for key, value in data.items():
            ...
            if key in self.attributes.keys():
                ...
                if key_type is Path and type(value) is str:
                    ...
                    setattr(self, key, parent.get_emulator(value))
                ...
                elif key_type is list and type(value) is str and value:
                    ...
                    setattr(
                        self, key, date(int(year), int(month), int(day)))
                    ...
                    if result is not None:
                        ...
                        setattr(self, key, timedelta(hours=int(hours),
                                                     minutes=int(minutes),
                                                     seconds=int(seconds)))

                elif key_type is str and value:
                    setattr(self, key, value)
    ...
    def copy(self, filename, parent=None):
        ...
        filename : pathlib.Path
            Game file path
        parent : gem.engine.api.GEM, optional
            API instance (Default: None)
    ...
    def reset(self):
        ...
        # FIXME: Environment variables are not reset since I remove the API
        #        parameter from Game constructor
        self.__init_attributes()
