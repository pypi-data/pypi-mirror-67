...
async def readline(pipe: Any) -> Tuple[OutputType, Optional[str]]:
    """-"""

    data = await pipe.readline()
    if data:
        line = str(data.decode('ascii').rstrip())
        if line.strip().startswith("INFO "):
            line = line.replace("INFO ", "")
            return (OutputType.STDOUT, line)
        elif line.strip().startswith("ERROR "):
            line = line.replace("ERROR ", "")
            return (OutputType.STDERR, line)
        else:
            if line.strip().startswith("DEBUG "):
                line = line.replace("DEBUG ", "")
            return (OutputType.DEBUG, line)

    return (OutputType.DEBUG, None)
...
async def start_subprocess(
        ...
        use_input: bool = False) -> Tuple[ProcessHandle, ProcessPipes]:
    """-"""
