...
class Dataset():
    ...
    """
    def __init__(self, name, id, type_metadata=None, URI=None,
                 human_description=None, marginals=None, columns=None):
        self.name = name
        self.id = id
    ...
    def _plot_marginal_feature(self, marginal_feature, width=1.5, heigth=1.5):
        ...
        if distrib:
            ...
            if len(distrib_s) > 5:
                ...
                distrib_s.append({'name': f'Other ({others_count})',
        ...
        if cumul:
            try:
                plt.fill_between([vp['value'] for vp in cumul],
...
class Client:
    ...
    def fetch_dataset_by_name(self, name):
        ...
        """
        try:
            request = requests.get(f'{self.base_url}/datasets/name/{name}',
                                   cookies=self.cookies, verify=self.verify)
            dataset = pickle.loads(request.content)
            return dataset
        except Exception:
    ...
    def fit(self, transform_def, keras_model_def, x=None, y=None,
            non_DP_training=False, **kwargs):
        """ NEW SIGNATURE!
        x and y are sarus datasets
        ...
        request = requests.post(
            f'{self.base_url}/fit',
            data=cloudpickle.dumps((transform_def, keras_model_def, x,
                                    non_DP_training, kwargs)),
            headers={'Content-Type': 'application/octet-stream'},
            cookies=self.cookies, verify=self.verify)
        if request.status_code > 200:
            raise Exception(f"Error while training the model.\
