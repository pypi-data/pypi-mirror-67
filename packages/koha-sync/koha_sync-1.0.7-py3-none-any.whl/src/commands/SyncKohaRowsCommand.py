    ...
    def execute(self):
        ...
        try:
            with DBPool.get_connection(True).cursor() as cursor:
                ...
                url = "{base}/{path}".format(base=Config.config['target']['url_add'], path=self.add)

                post_rows_req = requests.post(url, json={
                ...
                if post_rows_req.status_code is not 200:
                    logger.warning(f"Request status was not 200 : {vars(post_rows_req)}")
