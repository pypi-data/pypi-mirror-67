...
import os, sys, tempfile, shutil, json, subprocess, tarfile, glob, platform
import jsonschema
import waflib
...
def parse_pkg_recipe(conf, fname):
    ...
    for pkg in packages:
        i += 1
        for k in ("description", "requires", "name", "ignores", "excludes", "includes", "translations"):
            if k in pkg:
...
class PackageContext(Build.InstallContext):
    ...
    def package(self, srcdir: str, tmp: str, env: dict):
        ...
        if len(includes):
            paths = []
            for wildcard in includes:
                for w in ([wildcard], ['**', wildcard]):
                    paths += list(glob.glob(os.path.join(destdir, *w)))
            for root, _, files in os.walk(destdir):
                for f in files:
                    path = os.path.join(root, f)
                    if path not in paths and os.path.exists(path):
                        if os.path.isdir(path):    shutil.rmtree(path)
                        elif os.path.islink(path): os.unlink(path)
                        elif os.path.isfile(path): os.remove(path)

        # (2b) remove files matching excludes pattern
        else:
            for wildcard in env['PKG_EXCLUDES']:
                for w in ([wildcard], ['**', wildcard]):
                    for path in glob.glob(os.path.join(destdir, *w)):
        ...
        # (4) strip debug/uneeded symbols from binaries and shared libraries
        self.strip_symbols(destdir)
        
        # (5) create actual package
    ...
    def rpm_create(self, tmp: str, env: dict, destdir: str, release: str):
        ...
        # (5) create actual RPM and save result
